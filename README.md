...
## Installasi Otomatis
```
sysctl -w net.ipv6.conf.all.disable_ipv6=1 && sysctl -w net.ipv6.conf.default.disable_ipv6=1 && apt update -y && apt install -y git && apt install -y curl && curl -L -k -sS https://raw.githubusercontent.com/mudziboy/Autobot/main/start -o start && bash start sellvpn && [ $? -eq 0 ] && rm -f start
```
...
&& curl -L -k -sS https://raw.githubusercontent.com/mudziboy/Autobot/main/start2 -o start2 \
&& dos2unix start2 \
&& bash start2 sellvpn \
&& [ $? -eq 0 ] && rm -f start2
