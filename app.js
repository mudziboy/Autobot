...
if (typeof GROUP_ID !== 'undefined' && GROUP_ID) {
  // Gunakan escapeMarkdownV2 jika Anda memiliki fungsi tersebut, jika tidak, gunakan escapeMarkdown.
  const escapeFn = typeof escapeMarkdownV2 === 'function' ? escapeMarkdownV2 : escapeMarkdown;
  
  const adminMention = adminCtx.from.username ? `@${escapeFn(adminCtx.from.username)}` : escapeFn(adminCtx.from.first_name);
  const userMention = userCtx?.from.username ? `@${escapeFn(userCtx.from.username)}` : escapeFn(userCtx?.from.first_name || `ID ${user_id}`);
  const nominal = escapeFn(amount.toLocaleString('id-ID'));
  const waktu = escapeFn(new Date().toLocaleString('id-ID'));

  // Menghilangkan formatting tebal (*) untuk mengurangi resiko parsing error
  const notifGrup = `
━━━━━━━━━━━━━━━━━━━━━━━
...
━━━━━━━━━━━━━━━━━━━━━━━
  ...
  // Menggunakan parse_mode 'Markdown' saja, atau bahkan tanpa parse_mode jika error masih muncul
