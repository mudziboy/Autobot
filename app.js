...
// Formula yang sering digunakan untuk REQUEST (bukan callback): ref_kode + apikey + amount
function createVioletSignature(ref_kode, amount, isCallback = false) { 
    const apikey = VIOLET_API_KEY.trim();
    const secret_key = VIOLET_SECRET_KEY.trim();
    
    let dataToHash;
    
    // Jika untuk callback, gunakan formula sederhana (ref_kode saja)
    if (isCallback) {
        dataToHash = ref_kode; 
        return crypto.createHmac('sha256', secret_key).update(dataToHash).digest('hex');
    }
    
    // Jika untuk REQUEST Transaksi (PENTING!)
    // Formula umum API Payment Gateway: ref_kode + apikey + amount
    dataToHash = ref_kode + apikey + String(amount);
    
    // Kunci HMAC yang digunakan untuk REQUEST adalah SECRET KEY
    return crypto.createHmac('sha256', secret_key).update(dataToHash).digest('hex');
}

// GANTI SELURUH FUNGSI requestVioletTransaction DENGAN VERSI DEBUG DAN URLSearchParams

// app.js (Sekitar Baris 440)
            ...
            if (qrMessage) {
                 ...
                 await dbRunAsync(
                    `INSERT INTO pending_deposits (unique_code, user_id, amount, original_amount, timestamp, status, qr_message_id)
                     VALUES (?, ?, ?, ?, ?, ?, ?)`,
                    [refId, userId, totalBayarRaw, amount, Date.now(), 'pending_auto', qrMessage.message_id] 
                 );
...
// app.js (GANTI SELURUH BLOK app.post('/callback.php', ...) DENGAN INI)
// ==========================================================================================
app.post('/callback.php', async (req, res) => {
    const data = req.body;
    
    // 🚨 DEBUG 1: Pengecekan Payload Diterima
    logger.info(`[CALLBACK] VIOLETMEDIA Payload: ${JSON.stringify(data)}`);
    
    const sentSignature = req.headers['x-callback-signature'];
    const remoteIp = req.headers['x-forwarded-for'] || req.socket.remoteAddress;

    logger.info(`[CALLBACK] VIOLETMEDIA Callback diterima dari IP: ${remoteIp}. Ref: ${data.ref}, API Status: ${data.status}. Signature Header: ${sentSignature ? 'ADA' : 'TIDAK ADA'}`);

    // PENTING: Panggilan Signature untuk CALLBACK (asumsi formula sederhana)
    const requiredSignature = createVioletSignature(data.ref, null, true); 

    // Ganti data.signature dengan sentSignature yang diambil dari header
    if (requiredSignature !== sentSignature) { 
        // 🚨 DEBUG 2: Pengecekan Signature Gagal
        logger.error(`❌ VIOLETMEDIA Callback gagal, signature tidak cocok untuk ref: ${data.ref}. Expected: ${requiredSignature}, Received: ${sentSignature}`);
        return res.status(400).json({ status: false, message: 'Invalid Signature' });
    }
    
    logger.info(`[CALLBACK] Signature Cocok untuk Ref: ${data.ref}`);

    const refId = data.ref;
    const amountPaid = Number(data.nominal); // ✅ FIX: Konversi ke angka
    ...
    logger.info(`[CALLBACK] User ID berhasil diurai: ${userId}. Nominal: ${amountPaid}`);

    try {
        // Cek status deposit di DB lokal
        const row = await dbGetAsync('SELECT status, qr_message_id FROM pending_deposits WHERE unique_code = ?', [refId]);

        if (!row) {
            logger.warn(`⚠️ VIOLETMEDIA Callback: RefId ${refId} tidak ditemukan di pending_deposits (Mungkin sudah selesai/expired).`);
            return res.json({ status: true });
        }
        
        logger.info(`[CALLBACK] Status DB Lokal: ${row.status}. Status API: ${data.status}`);

        // Cek apakah sudah sukses sebelumnya
        if (row.status === 'success') {
            logger.warn(`⚠️ VIOLETMEDIA Callback: RefId ${refId} sudah diproses.`);
            return res.json({ status: true });
        }

        if (data.status === 'success') {
            
            // 🚨 DEBUG 3: Menambah Saldo
            logger.info(`[CALLBACK] Memproses penambahan saldo sebesar ${amountPaid} ke user ${userId}...`);
            await dbRunAsync('UPDATE users SET saldo = saldo + ? WHERE user_id = ?', [amountPaid, userId]);
            
            // 🚨 DEBUG 4: Update Status DB
            await dbRunAsync('UPDATE pending_deposits SET status = "success" WHERE unique_code = ?', [refId]);
            logger.info(`[CALLBACK] Status DB diupdate ke SUCCESS.`);
            ...
            // 4. Hapus pesan QRIS dari chat user
            if (row.qr_message_id) {
                logger.info(`[CALLBACK] Menghapus pesan QRIS ID: ${row.qr_message_id}`);
                await bot.telegram.deleteMessage(userId, row.qr_message_id).catch(e => {
                    logger.warn(`Gagal hapus pesan QRIS ${row.qr_message_id}: ${e.message}`);
                });
            }
            ...
            logger.info(`✅ VIOLETMEDIA Success for user ${userId}. Amount: ${amountPaid}. Transaksi Selesai.`);
            
            return res.json({ status: true });

        } else if (data.status === 'kadaluarsa' || data.status === 'failed') {
            const status = data.status;
            // 🚨 DEBUG 5: Status Kadaluarsa/Failed
            logger.info(`[CALLBACK] Status API: ${status}. Memproses update DB dan notifikasi.`);
            await dbRunAsync('UPDATE pending_deposits SET status = ? WHERE unique_code = ?', [status, refId]);
            
            // Kirim notifikasi expired/failed
            if (status === 'kadaluarsa') {
                await sendTopupNotification(userId, amountPaid, false, 'kadaluarsa');
            }
            
            // Hapus pesan QRIS dari chat user
            if (row.qr_message_id) {
                await bot.telegram.deleteMessage(userId, row.qr_message_id).catch(e => {
                    logger.warn(`Gagal hapus pesan QRIS ${row.qr_message_id} (expired): ${e.message}`);
                });
            }
            
            logger.info(`⚠️ VIOLETMEDIA Transaction ${refId} set to ${status}.`);
            return res.json({ status: true });
        }
        
        // Untuk status lain (misalnya pending/paid)
        logger.info(`[CALLBACK] Status ${data.status} Diterima. Update DB ke status baru.`);
        await dbRunAsync('UPDATE pending_deposits SET status = ? WHERE unique_code = ?', [data.status, refId]);
        return res.json({ status: true });

    } catch (e) {
        // 🚨 DEBUG 6: Gagal Eksekusi DB
        logger.error(`❌ VIOLETMEDIA Callback Gagal Eksekusi DB/Notif untuk ${refId}: ${e.message}. Stack: ${e.stack}`);
