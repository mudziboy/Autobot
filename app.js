...
// 💡 PENAMBAHAN KRITIS: DEFINE URL API ASLI
const VIOLET_API_BASE_URL = 'https://violetmediapay.com/api/live/';
...
// ===========================
// 📦 CALLBACK VIOLET MEDIA (Versi Final 20 Oktober 2025)
// ===========================
app.post("/callback.php", async (req, res) => {
  ...
  // 🧾 Daftar IP yang diizinkan (termasuk test lokal)
  const ALLOWED_IPS = [
    "127.0.0.1",               // loopback IPv4 (test)
    "::1",                     // loopback IPv6 (test)
    "103.168.146.138",         // IP VPS kamu
  ...
  try {
    ...
    // 🧾 Ambil signature dari header (prioritas tinggi)
    ...
    // 🔑 Hitung ulang signature lokal (secret key + raw body)
    // 💡 KRITIS: Menggunakan formula request karena formula callback docs sangat aneh
    const amount = data.nominal || 0; // Ambil nominal dari callback data
    
    // Formula: hash_hmac('sha256', ref_kode + apikey + amount, secret_key)
    const dataToHash = ref + API_KEY + String(amount); 
    ...
    // 🔍 Normalisasi IP agar cocok walau format IPv4-mapped (::ffff:)
    ...
    log(`→ clientIp detected: ${clientIp} (normalized: ${normalizedIp})`);
...
// === START: CALLBACK LOGIC VIOLETMEDIA ===
try {
  // Ambil ID grup admin jika tersedia di vars.json
  const GROUP_ID = (global.vars?.GROUP_ID || process.env.GROUP_ID || "").trim();
  ...
  db.get(
    ...
    (err, row) => {
      if (err) {
        log(`💥 DB Error saat mencari ref ${ref}: ${err.message}`);
        return;
      }
      ...
      // 1️⃣ Update status transaksi menjadi 'paid'
      db.run(
        "UPDATE pending_deposits SET status = 'paid' WHERE unique_code = ?",
        [ref],
        function (updateErr) {
          if (updateErr) {
            log(`💥 Gagal update status untuk ref ${ref}: ${updateErr.message}`);
            return;
          }
          log(`💾 Status transaksi ${ref} diperbarui menjadi 'paid'.`);
          ...
          db.run(
            ...
            function (balanceErr) {
              if (balanceErr) {
                log(`💥 Gagal update saldo user ${userId}: ${balanceErr.message}`);
                return;
              }
              ...
              // 3️⃣ Kirim notifikasi ke user (non-async, pakai .then/.catch)
              const userMsg =
                `💰 *Top-up Berhasil!*\n\n` +
                `Kode transaksi: \`${ref}\`\n` +
              ...
              bot.telegram.sendMessage(userId, userMsg, { parse_mode: "Markdown" })
                .then(() => {
                  log(`📨 Notifikasi Telegram terkirim ke user ${userId}.`);
                })
                .catch((tgErr) => {
                  log(`⚠️ Gagal kirim pesan Telegram ke user ${userId}: ${tgErr.message}`);
                });

              // 4️⃣ Kirim notifikasi ke grup admin jika ID tersedia
              if (GROUP_ID) {
                const adminMsg =
                  `📢 *Transaksi Sukses (Otomatis)*\n\n` +
                  `👤 User ID: ${userId}\n` +
                  `💸 Jumlah: Rp ${amount.toLocaleString("id-ID")}\n` +
                  `🔖 Ref: \`${ref}\`\n` +
                  `🕒 ${new Date().toLocaleString("id-ID")}`;

                bot.telegram.sendMessage(GROUP_ID, adminMsg, { parse_mode: "Markdown" })
                  .then(() => {
                    log(`📤 Notifikasi grup admin terkirim (ID: ${GROUP_ID}).`);
                  })
                  .catch((err) => {
                    log(`⚠️ Gagal kirim notifikasi grup admin: ${err.message}`);
                  });
              }
            }
          );
...
// 💡 Tambahkan deklarasi untuk global.depositState dan global.pendingDeposits
global.depositState = {}; // Baris baru
global.pendingDeposits = {}; // Baris baru
...
// ===== VIOLETMEDIA UTILITY (Modifikasi Signature) =====

// PENTING: Perbaiki formula signature yang lebih fleksibel
function createVioletSignature(ref_kode, amount, type) { 
    const secret_key = VIOLET_SECRET_KEY.trim();
    const apikey = VIOLET_API_KEY.trim();
    
    // --- MODE CALLBACK (VERIFIKASI) ---
    if (type === 'callback') {
        // Formula Docs CALLBACK: hash_hmac('sha256', $refid, $apikey)
        // **CATATAN: Signature API Violet Media menggunakan apikey sebagai secret**
        // Jika dokumen callback Anda menggunakan $refid sebagai pesan, dan $apikey sebagai secret:
        // return crypto.createHmac('sha256', apikey).update(ref_kode).digest('hex'); 
        
        // Namun, jika API Key dan Secret Key dipisahkan: kita coba versi yang lebih umum
        // Versi Umum (Request): $ref_kode.$apikey.$amount dengan SECRET_KEY
        return crypto.createHmac('sha256', secret_key)
            .update(ref_kode + apikey + String(amount))
            .digest('hex');
    }
    
    // --- MODE REQUEST TRANSAKSI (QRIS/CREATE) ---
    // Formula Docs REQUEST: hash_hmac('sha256', ref_kode + apikey + amount, secret_key)
...
async function requestVioletTransaction(userId, amount, email, phone, paymentMethodCode) {
    ...
    const signature = createVioletSignature(ref_kode, String(amount)); // Menggunakan formula baru
    
    // --- 1. GUNAKAN OBJECT JSON (Sesuai Docs) ---
    const postdata = {
        api_key: VIOLET_API_KEY.trim(),
        secret_key: VIOLET_SECRET_KEY.trim(),
        channel_payment: paymentMethodCode,
        ref_kode: ref_kode,
        nominal: amount,
        cus_nama: email.split('@')[0],
        cus_email: email,
        cus_phone: phone,
        produk: `TopUp Saldo ${NAMA_STORE} - ${amount}`,
        url_redirect: VIOLET_CALLBACK_URL.replace('callback', 'redirect'),
        url_callback: VIOLET_CALLBACK_URL,
        expired_time: Math.floor(Date.now() / 1000) + 3600, // 1 jam expired (dalam detik)
        signature: signature
    };
    // ----------------------------------

    try {
        const url = `${VIOLET_API_BASE_URL}create`; // 💡 Menggunakan URL base baru
        
        // --- 2. KONFIGURASI AXIOS: Application JSON (Sesuai Docs) ---
        const config = {
            headers: {
        ...
        logger.info(`[VIOLET TRANS] Mengirim transaksi Ref: ${ref_kode}, Amount: ${amount}. Payload: ${JSON.stringify(postdata)}`);
        
        const response = await axios.post(url, postdata, config); // Kirim postdata (JSON)
...
bot.action('addsaldo_user', async (ctx) => {
  try {
    ...
    const users = await new Promise((resolve, reject) => {
      // 💡 Ambil juga username agar bisa ditampilkan
      db.all('SELECT id, user_id, username, first_name FROM Users LIMIT 20', [], (err, users) => {
        if (err) {
    ...
    for (let i = 0; i < users.length; i += 2) {
      const row = [];
      const user1 = users[i];
      const name1 = user1.username ? `@${user1.username}` : user1.first_name || user1.user_id;

      row.push({
        text: name1,
        callback_data: `add_saldo_select_${user1.id}` // Ganti ke format yang lebih jelas
      });
      
      if (i + 1 < users.length) {
        const user2 = users[i + 1];
        const name2 = user2.username ? `@${user2.username}` : user2.first_name || user2.user_id;

        row.push({
          text: name2,
          callback_data: `add_saldo_select_${user2.id}`
        });
      }
      buttons.push(row);
    }
    ...
    replyMarkup.inline_keyboard.push([{ text: '⬅️ Kembali', callback_data: 'admin_system_menu' }]);


    await ctx.editMessageText('📊 *Silakan pilih user untuk menambahkan saldo (halaman 1):*', {
      reply_markup: replyMarkup,
      parse_mode: 'Markdown'
    });
  } catch (error) {
    logger.error('❌ Kesalahan saat memulai proses tambah saldo user:', error);
    await ctx.reply(`❌ *Terjadi kesalahan saat memuat daftar user: ${error.message || error}*`, { parse_mode: 'Markdown' });
  }
});

// Baris 5576 (Ganti fungsi yang ada dengan kode ini)
...
bot.action(/next_users_(\d+)/, async (ctx) => {
  ...
  try {
    ...
    const users = await new Promise((resolve, reject) => {
      // 💡 Ambil juga username dan first_name
      db.all(`SELECT id, user_id, username, first_name FROM Users LIMIT 20 OFFSET ${offset}`, [], (err, users) => {
        if (err) {
          logger.error('❌ Kesalahan saat mengambil daftar user:', err.message);
          return reject('⚠️ *PERHATIAN! Terjadi kesalahan saat mengambil daftar user.*');
        }
    ...
    for (let i = 0; i < users.length; i += 2) {
      ...
      row.push({
        text: name1,
        callback_data: `add_saldo_select_${user1.id}`
      });
      if (i + 1 < users.length) {
        const user2 = users[i + 1];
        const name2 = user2.username ? `@${user2.username}` : user2.first_name || user2.user_id;
    ...
    await ctx.editMessageText(`📊 *Silakan pilih user untuk menambahkan saldo (halaman ${currentPage + 1}):*`, {
      reply_markup: replyMarkup,
      parse_mode: 'Markdown'
    });
  } catch (error) {
    logger.error('❌ Kesalahan saat memproses next users:', error);
    await ctx.reply(`❌ *Terjadi kesalahan saat memproses next users: ${error.message || error}*`, { parse_mode: 'Markdown' });
  }
});
...
bot.action(/prev_users_(\d+)/, async (ctx) => {
  ...
  try {
    logger.info(`Previous users process started for page ${currentPage + 1}`);
    await ctx.answerCbQuery();
    ...
    if (currentPage > 0) {
      navigationButtons.push({
        text: '⬅️ Back',
        callback_data: `next_users_${currentPage - 1}` // Menggunakan next_users untuk mundur
      });
    }
    if (offset + 20 < totalUsers) {
      navigationButtons.push({
    ...
    await ctx.reply(`❌ *Terjadi kesalahan saat memproses previous users: ${error.message || error}*`, { parse_mode: 'Markdown' });
  }
});
...
`.trim();
    ...
    const keyboard = [
      [{ text: '💸 Top-up Otomatis (Payment Gateway)', callback_data: 'topup_auto_start' }],
      [{ text: '👤 Top-up Manual (Admin Verifikasi)', callback_data: 'topup_manual_start' }],
      [{ text: '🔙 Kembali', callback_data: 'send_main_menu' }]
    ];
    ...
    logger.error('❌ Kesalahan saat menampilkan menu top-up:', error);
    await ctx.reply('❌ *Gagal memproses. Silakan coba lagi nanti.*', { parse_mode: 'Markdown' });
  }
});
...
bot.action('topup_manual_start', async (ctx) => {
  try {
    ...
    const qrisImageUrl = 'https://raw.githubusercontent.com/mudziboy/mdzitr/refs/heads/main/qris.png';
...
bot.on(['photo', 'document'], async (ctx) => {
  ...
  await ctx.reply('✅ Bukti transfer diterima, diteruskan ke admin untuk verifikasi.');
  const forwardedMsg = await ctx.forwardMessage(adminId, ctx.chat.id, ctx.message.message_id);

  await bot.telegram.sendMessage(
    adminId,
    `📥 User @${ctx.from.username || userId} mengirim bukti top-up.\n\nUserID: ${userId}`,
    {
...
const handleAdminCallback = async (ctx) => {
  ...
  if (data.startsWith('verif_')) {
    ...
    await ctx.reply(`💰 Masukkan nominal top-up untuk User ${userId}:`, {
  ...
  // === ADMIN TOLAK ===
    ...
    await safeSend(bot, userCtx.chat.id, '❌ Top-up Anda *ditolak* oleh admin, Silahkan kirim bukti valid.', { parse_mode: 'Markdown' });
    // await userCtx.deleteMessage(global.depositState[userId].messageId).catch(() => {}); // Tidak perlu lagi messageId
    delete global.depositState[userId];

    await ctx.editMessageText(`❌ Top-up user ${userId} ditolak oleh admin.`);
    await ctx.answerCbQuery();
    return true;
  }
  ...
  // === TOMBOL NOMOR ===
    ...
    if (key === 'ok') {
      ...
      if (isNaN(amount) || amount <= 0) return ctx.answerCbQuery('❌ Nominal tidak valid.');
      await handleTopup(userId, amount, ctx);
      await ctx.answerCbQuery();

    } else if (key === 'clear') {
      global.depositState[userId].inputNominal = '';
      await ctx.editMessageText(`💰 Nominal yang diketik: Rp0`, { reply_markup: { inline_keyboard: keyboard_nomor_topup(userId) } });
      await ctx.answerCbQuery();
      ...
      if (currentNominal.length < 10) { // Batasi maksimal 10 digit
        ...
        await ctx.editMessageText(`💰 Nominal yang diketik: Rp${newNominal}`, { reply_markup: { inline_keyboard: keyboard_nomor_topup(userId) } });
      }
      await ctx.answerCbQuery();
    }
    return true;
...
await adminCtx.editMessageText(`✅ Top-up User ${userId} sebesar Rp${amount.toLocaleString('id-ID')} berhasil diverifikasi.`);
...
━━━━━━━━━━━━━━━━━━━━━━━
    ...
    logger.error(`❌ handleTopup() error untuk user ${userId}:`, err.message);
    await adminCtx.reply(`❌ *Gagal memproses top-up untuk user ${userId}.*`);
    if (userCtx) await safeSend(bot, user_id, '❌ *Terjadi kesalahan fatal saat menambahkan saldo. Harap hubungi admin.*', { parse_mode: 'Markdown' });
    delete global.depositState[userId];
  }
}
...
// Lokasi: Letakkan di sekitar Baris 7050 (Sebelum keyboard_nomor_topup)
async function handleAutoDepositInput(ctx, userId, data) {
    let currentAmount = global.depositState[userId].amount;
    const MIN_AMOUNT = 10000; // Minimal 10.000 (disesuaikan dengan bisnis)
    ...
    if (data === 'delete') {
        ...
        // Baris 7234 (Ganti bagian pengecekan channelList di dalam handleAutoDepositInput)

        // 1. Fetch Channel Pembayaran dari VioletMedia
        const channelList = await getVioletPaymentChannels();
        
        // --- KRITIS: PERIKSA STATUS UTAMA DAN DATA SEBELUM PENCARIAN ---
        if (!channelList || channelList.status !== true) {
            // Log error API spesifik dari Violet Media (jika ada)
            const apiError = channelList.data?.status || channelList.message || 'Error otentikasi API.';
            delete global.depositState[userId];
            logger.error(`❌ VIOLET API CHANNEL FAILED: ${apiError}`);
            return await ctx.reply(`❌ Maaf, layanan pembayaran sedang tidak tersedia.\n*Pesan Error: ${apiError}*`, { parse_mode: 'Markdown' });
        }
        
        // Memastikan channelList.data adalah array (mengatasi error "is not a function")
        if (!Array.isArray(channelList.data)) {
            delete global.depositState[userId];
            logger.error(`❌ VIOLET API RESPON BERMASALAH: Data channel bukan array.`);
            return await ctx.reply('❌ Terjadi kesalahan saat membaca data channel pembayaran.', { parse_mode: 'Markdown' });
        }

        // --- FILTER KODE LANJUTAN ---
        const TARGET_CODE = 'QRIS'; 
        
        // Cari channel yang sesuai dengan Kode TARGET dan statusnya AKTIF
        const finalChannel = channelList.data
            .find(c => c.status === 'Aktif' && c.Kode === TARGET_CODE); 
        ...
        const keyboard = [[
            { text: `💳 ${finalChannel.nama} - Biaya: ${finalChannel.biaya}%`, callback_data: `select_channel_${finalChannel.Kode}` } 
        ]];
        ...
        // Atur timeout baru untuk pemilihan channel
         global.depositState[userId].timeout = setTimeout(() => {
            if (global.depositState[userId] && global.depositState[userId].action === 'await_channel_select') {
                safeDeleteMessage(bot, userId, global.depositState[userId].messageId);
                delete global.depositState[userId];
                safeSend(bot, userId, '⌛ Sesi pemilihan channel top-up expired.', { parse_mode: 'Markdown' });
            }
        }, 5 * 60 * 1000); 
...
// ===== AKHIR HANDLE AUTO DEPOSIT INPUT =====
...
bot.action('topup_auto_start', async (ctx) => {
    ...
    const message = await ctx.reply('💰 *Masukkan nominal top-up (minimal Rp 10.000):*', {
        parse_mode: 'Markdown',
    ...
    global.depositState[userId].timeout = setTimeout(() => {
        if (global.depositState[userId] && global.depositState[userId].action === 'await_auto_amount') {
            safeDeleteMessage(bot, userId, global.depositState[userId].messageId);
            delete global.depositState[userId];
            safeSend(bot, userId, '⌛ Sesi top-up otomatis expired.', { parse_mode: 'Markdown' });
        }
    }, 10 * 60 * 1000);
});
    ...
    try {
        ...
        if (response.status === true || response.status === 'success') {
            ...
            global.depositState[userId] = {
                ...
                expired: expiredAtRaw * 1000 // 💡 KRITIS: Asumsi expired_time dari VM adalah UNIX Timestamp (detik). Konversi ke milidetik.
            };
            ...
            const petunjukText = data.petunjuk 
                ...
                : 'Silakan scan QRIS di atas atau klik tombol Link Pembayaran.';

            // Template Invoice untuk Caption (Menggunakan Markdown TOLERAN)
            const invoiceText = `
🧾 *INVOICE TOP-UP OTOMATIS*
--------------------------------------
...
`.trim(); // Menggunakan tanda hubung (-) yang seharusnya aman di Plain Text/Markdown
            ...
            if (qrMessage) {
                 const expiredTimestampMs = expiredAtRaw * 1000; // 💡 Gunakan nilai yang dikonversi
                 const timeoutDuration = expiredTimestampMs - Date.now();
...
bot.action(/add_saldo_select_(\d+)/, async (ctx) => {
  const rowId = ctx.match[1]; // Ini adalah ID dari tabel users (bukan user_id Telegram)
  
  const user = await dbGetAsync('SELECT user_id FROM users WHERE id = ?', [rowId]);
  if (!user) {
    return ctx.reply('❌ User tidak ditemukan.');
  }
  
  logger.info(`User ${ctx.from.id} memilih untuk menambahkan saldo user dengan ID Telegram: ${user.user_id}`);
  userState[ctx.chat.id] = { step: 'add_saldo', userId: user.user_id, saldo: '' }; // Inisialisasi saldo kosong

  await ctx.editMessageText('📊 *Silakan masukkan jumlah saldo yang ingin ditambahkan:*\n\nJumlah saldo saat ini: *Rp 0*', {
    reply_markup: { inline_keyboard: keyboard_nomor() },
    parse_mode: 'Markdown'
  });
});
...
// Baris 5917 (Ganti fungsi yang ada dengan kode ini)
...
if (isAdmin && (data.startsWith('verif_') || data.startsWith('tolak_') || data.startsWith('keyboard_'))) {
  ...
  if (userStateData) {
    switch (userStateData.step) {
      // 💡 Tambahkan logika untuk Add Saldo menggunakan user_id
      case 'add_saldo': 
          // Ambil user_id dari callback data jika belum ada di state
          if (!userStateData.userId && data.startsWith('add_saldo_')) {
              const rowId = data.split('_')[2];
              const user = await dbGetAsync('SELECT user_id FROM users WHERE id = ?', [rowId]);
              if (user) userStateData.userId = user.user_id;
          }
 ...
 if (data.startsWith('restore_uploaded_file::')) {
  ...
  try {
    ...
    // logRestoreAction('restore_upload', fileName, ctx.from.username, ctx.from.id); // <--- Hapus jika logRestoreAction tidak didefinisikan
  } catch (err) {
  ...
  try {
    ...
    // logRestoreAction('delete_upload', fileName, ctx.from.username, ctx.from.id); // <--- Hapus jika logRestoreAction tidak didefinisikan
  } catch (err) {
...
async function getVioletPaymentChannels() {
    try {
        const url = `${VIOLET_API_BASE_URL}channel-payment`; // 💡 Menggunakan URL base baru
        
        // --- 1. GUNAKAN OBJECT JSON (Sesuai Docs) ---
        const postdata = {
            api_key: VIOLET_API_KEY.trim(),
            secret_key: VIOLET_SECRET_KEY.trim(),
            channel_payment: 'list' // Sesuai docs
        };
        // ----------------------------------
        ...
        const config = {
            headers: {
                'Content-Type': 'application/json' // 💡 KRITIS: Diubah ke JSON
            },
            timeout: 30000 
        };
...
// 💡 Modifikasi atau Penambahan fungsi Handlers Admin (Di dekat Baris 5488 - 5849)
...
async function handleAddSaldo(ctx, userStateData, data) {
  ...
  if (data === 'delete') {
    ...
    if (currentSaldo.length === 0 || parseInt(currentSaldo) <= 0) {
      return await ctx.answerCbQuery('⚠️ *Jumlah saldo harus lebih dari 0!*', { show_alert: true });
    }

    try {
      await updateUserSaldo(userId, parseInt(currentSaldo));
      const user = await dbGetAsync('SELECT username, saldo FROM users WHERE user_id = ?', [userId]);
      
      const newSaldo = user?.saldo || 0;
      
      ctx.reply(
        `✅ *Saldo user ${userId} berhasil ditambahkan.*\n` +
        `💰 *Jumlah Saldo: Rp ${parseInt(currentSaldo).toLocaleString('id-ID')}*\n` +
        `💸 *Saldo Terbaru: Rp ${newSaldo.toLocaleString('id-ID')}*`, 
        { parse_mode: 'Markdown' }
      );
    } catch (err) {
      logger.error('❌ Error saat menambahkan saldo:', err.message);
  ...
  const displayAmount = parseInt(currentSaldo || 0).toLocaleString('id-ID');
  const newMessage = `📊 *Silakan masukkan jumlah saldo yang ingin ditambahkan:*\n\nJumlah saldo saat ini: *Rp ${displayAmount}*`;
  ...
  try {
    if (ctx.callbackQuery.message.text !== newMessage) {
      ...
      logger.error('Error saat edit message tambah saldo:', e.message);
  }
}

async function handleEditBatasCreateAkun(ctx, userStateData, data) {
  await handleEditNumericField(ctx, userStateData, data, 'batasCreateAkun', 'batas create akun', 'UPDATE Server SET batas_create_akun = ? WHERE id = ?');
}

async function handleEditTotalCreateAkun(ctx, userStateData, data) {
  await handleEditNumericField(ctx, userStateData, data, 'totalCreateAkun', 'total create akun', 'UPDATE Server SET total_create_akun = ? WHERE id = ?');
}

async function handleEditiplimit(ctx, userStateData, data) {
  await handleEditNumericField(ctx, userStateData, data, 'iplimit', 'limit IP', 'UPDATE Server SET iplimit = ? WHERE id = ?');
}

async function handleEditQuota(ctx, userStateData, data) {
  await handleEditNumericField(ctx, userStateData, data, 'quota', 'quota', 'UPDATE Server SET quota = ? WHERE id = ?');
}

async function handleEditAuth(ctx, userStateData, data) {
  await handleEditTextField(ctx, userStateData, data, 'auth', 'auth', 'UPDATE Server SET auth = ? WHERE id = ?', keyboard_full());
}

async function handleEditDomain(ctx, userStateData, data) {
  await handleEditTextField(ctx, userStateData, data, 'domain', 'domain', 'UPDATE Server SET domain = ? WHERE id = ?', keyboard_full());
}
...
async function handleEditHarga(ctx, userStateData, data) {
  ...
  if (data === 'delete') {
    ...
    if (currentAmount.length === 0 || parseFloat(currentAmount) <= 0) {
      return await ctx.answerCbQuery('⚠️ *Harga harus lebih dari 0!*', { show_alert: true });
    }
    const hargaBaru = parseFloat(currentAmount);
    try {
      await updateServerField(userStateData.serverId, hargaBaru, 'UPDATE Server SET harga = ? WHERE id = ?', 'harga');
      ctx.reply(`✅ *Harga server berhasil diupdate.*\n\n💰 *Harga Baru: Rp ${hargaBaru.toLocaleString('id-ID')}*`, { parse_mode: 'Markdown' });
    } catch (err) {
      logger.error('❌ Error saat update harga server:', err.message);
  ...
  try {
    if (ctx.callbackQuery.message.text !== newMessage) {
      ...
      if (e.description && e.description.includes('message is not modified')) return;
      logger.error('Error saat edit message harga:', e.message);
  }
}

async function handleEditNama(ctx, userStateData, data) {
  await handleEditTextField(ctx, userStateData, data, 'name', 'nama server', 'UPDATE Server SET nama_server = ? WHERE id = ?', keyboard_abc());
}

// --------------------------------------------------
// Fungsi Helper Generik untuk Edit Field Server
// --------------------------------------------------

async function handleEditNumericField(ctx, userStateData, data, field, fieldName, query) {
  let currentValue = userStateData[field] || '';

  if (data === 'delete') {
    currentValue = currentValue.slice(0, -1);
  } else if (data === 'confirm') {
    if (currentValue.length === 0 || parseInt(currentValue) < 0) {
      return await ctx.answerCbQuery(`⚠️ *${fieldName} tidak boleh kosong dan harus >= 0!*`, { show_alert: true });
    }
    const numericValue = parseInt(currentValue, 10);
    try {
      await updateServerField(userStateData.serverId, numericValue, query, fieldName);
      ctx.reply(`✅ *${fieldName} server berhasil diupdate.*\n\n🔢 *Nilai Baru: ${numericValue.toLocaleString('id-ID')}*`, { parse_mode: 'Markdown' });
    } catch (err) {
      logger.error(`❌ Error saat update ${fieldName}:`, err.message);
  ...
  const displayValue = parseInt(currentValue || 0).toLocaleString('id-ID');
  const newMessage = `📊 *Silakan masukkan ${fieldName} server baru:*\n\nNilai saat ini: *${displayValue}*`;
  ...
  try {
    if (ctx.callbackQuery.message.text !== newMessage) {
      ...
      if (e.description && e.description.includes('message is not modified')) return;
      logger.error(`Error saat edit message ${fieldName}:`, e.message);
  }
}

async function handleEditTextField(ctx, userStateData, data, field, fieldName, query, keyboardLayout) {
  let currentValue = userStateData[field] || '';
  ...
  if (data === 'delete') {
    ...
    try {
      ...
      ctx.reply(`✅ *${fieldName} server berhasil diupdate.*\n\n📄 *Nilai Baru: ${currentValue}*`, { parse_mode: 'Markdown' });
    } catch (err) {
      logger.error(`❌ Error saat update ${fieldName}:`, err.message);
      ctx.reply(`❌ *Terjadi kesalahan saat mengupdate ${fieldName} server.*`, { parse_mode: 'Markdown' });
    }
...
async function updateServerField(serverId, value, query, fieldName) {
  return new Promise((resolve, reject) => {
...
// Perbaikan fungsi helper updateUserSaldo untuk menggunakan user_id, bukan id (sesuai skema DB)
