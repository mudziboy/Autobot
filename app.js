...
if (typeof GROUP_ID !== 'undefined' && GROUP_ID) {
    // Karena MarkdownV2 sangat ketat, kita akan kirim sebagai Plain Text.
    // Kita tetap menggunakan escapeFn untuk membersihkan teks, tapi menghilangkan parse_mode.

    const escapeFn = typeof escapeMarkdownV2 === 'function' ? escapeMarkdownV2 : escapeMarkdown;
    
    // Kita gunakan data yang sudah di-escape, tetapi kirim sebagai plain text.
    const adminMention = adminCtx.from.username ? `@${escapeFn(adminCtx.from.username)}` : adminCtx.from.first_name;
    const userMention = userCtx?.from.username ? `@${escapeFn(userCtx.from.username)}` : userCtx?.from.first_name || `ID ${user_id}`;
    const nominal = amount.toLocaleString('id-ID'); // Tidak perlu escape karena tidak ada formatting
    const waktu = new Date().toLocaleString('id-ID');

    // Menghilangkan semua karakter formatting Markdown (*) untuk keamanan
    const notifGrup = `
━━━━━━━━━━━━━━━━━━━━━━━
...
━━━━━━━━━━━━━━━━━━━━━━━
    ...
    // PENTING: Panggil safeSend TANPA parse_mode untuk mengirim Plain Text
