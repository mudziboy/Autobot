...
// TOPUP otomatis: ask user to input amount
bot.action('topup_otomatis', async (ctx) => {
  await ctx.answerCbQuery().catch(()=>{});
  const uid = String(ctx.from.id);
  violetDepositState[uid] = { action: 'await_amount' };
  await ctx.reply('Masukkan nominal top-up (minimal Rp10.000). Contoh: 10000');
});
...
/* --- END Violet Media Top-Up Integration (ADDED) --- */
...
// ===== END OF MERGED FILE (auto-merged & patched) =====
