...
bot.action(/select_channel_(\w+)/, async (ctx) => {
    ...
    try {
        ...
        if (response.status === true || response.status === 'success') {
            ...
            // Perbaikan KRITIS: Akses 'data' (d kecil) karena requestVioletTransaction mengembalikan response.data
            ...
            // Template Invoice untuk Caption (Perbaikan Error Markdown '-')
            const invoiceText = `
...
`.trim();
            ...
            if (data.target) {
                ...
                try {
                 ...
                 // Fallback jika tidak ada link target (tidak mungkin terjadi pada QRIS)
            ...
            if (qrMessage) {
                 const expiredTimestamp = Date.parse(data.expired_time);
                 const timeoutDuration = expiredTimestamp - Date.now();

                 // Log DB sebelum set timeout
                 await dbRunAsync(
                    `INSERT INTO pending_deposits (unique_code, user_id, amount, original_amount, timestamp, status, qr_message_id)
                     VALUES (?, ?, ?, ?, ?, ?, ?)`,
                    [refId, userId, totalBayar, amount, Date.now(), 'pending_auto', qrMessage.message_id] 
                 );

                 // Set Timeout untuk menghapus pesan setelah expired (dengan buffer 1 menit)
                 global.depositState[userId].timeout = setTimeout(async () => {
                    // Gunakan bot.telegram.deleteMessage karena timeout tidak memiliki context (ctx)
                    await bot.telegram.deleteMessage(userId, qrMessage.message_id).catch(e => {
                        logger.warn(`Gagal menghapus pesan ${qrMessage.message_id} (mungkin sudah dihapus/terlalu lama): ${e.message}`);
                    });
                    
                    // Hapus state dan DB log terkait pending_deposits jika masih ada (optional)
                    // Hapus logika expired di sini
                    
                    logger.info(`🗑️ Pesan QRIS ${qrMessage.message_id} untuk ${userId} telah dihapus karena expired.`);

                 }, Math.max(30000, timeoutDuration + 60000)); // Minimal 30 detik, atau (expired + 1 menit)
