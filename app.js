...
function dlog(msg){ if(typeof DEBUG_LOG !== 'undefined' && DEBUG_LOG) console.log('[DEBUG]', msg); }
...
function safePm2Restart(name) {
  try {
    exec('pm2 restart ' + name, (err, stdout, stderr) => {
      if (err) {
        logger.error('❌ Gagal restart via PM2: ' + (err.message || err));
      } else {
        logger.info('✅ pm2 restart ' + name + ' executed');
      }
    });
  } catch (e) {
    logger.error('safePm2Restart exception: ' + e.message);
  }
}
