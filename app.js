...
// app.js (GANTI SELURUH BLOK app.post('/callback.php', ...) DENGAN INI - Sekitar Baris 5634)
// ==========================================================================================

app.post('/callback.php', async (req, res) => {
    
    // 🚨 KRITIS: Ambil signature dari header 'X-Callback-Signature' (headers otomatis lower-case)
    const sentSignature = req.headers['x-callback-signature'];
    
    // 🚨 KRITIS: Ambil raw body dari request (sudah diatur di middleware Express)
    ...
    // --- 1. VALIDASI SIGNATURE RAW BODY ---
    if (!rawBody) {
        logger.error(`❌ VIOLETMEDIA Callback gagal: Raw body tidak ditemukan!`);
        return res.status(400).json({ status: false, message: 'Raw body missing' });
    }
    
    // PANGGILAN SIGNATURE: Gunakan rawBody (argumen ketiga) untuk formula hashing yang benar
    const requiredSignature = createVioletSignature(null, null, rawBody); 
    ...
    logger.info(`[CALLBACK] Signature Cocok. User ID: ${userId}. Nominal: ${amountPaid}`);

    // --- 2. PROSES TRANSAKSI DB ---
    try {
        ...
        if (!row) {
            ...
            return res.json({ status: true }); // Tetap kirim 200 OK
        }
        ...
        if (data.status === 'success') {
            ...
            logger.info(`✅ VIOLETMEDIA Success for user ${userId}. Saldo ditambahkan.`);
            
            // 🚨 Response sukses untuk Payment Gateway
            return res.json({ status: true });
        ...
        // Response untuk status lain (misalnya pending/paid)
        await dbRunAsync('UPDATE pending_deposits SET status = ? WHERE unique_code = ?', [data.status, refId]);
        return res.json({ status: true });

    } catch (e) {
        logger.error(`❌ VIOLETMEDIA Callback Gagal Eksekusi DB/Notif untuk ${refId}: ${e.message}`);
        // 🚨 PENTING: Response Error Internal Server
