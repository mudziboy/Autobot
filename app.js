...
// ===== SETUP FILE USERS (VERSI FIX STABIL) =====
// Gunakan folder data khusus agar path absolut & konsisten
const dataDir = path.resolve('/root/BotVPN2/data'); // path absolut biar gak berubah
if (!fs.existsSync(dataDir)) {
  fs.mkdirSync(dataDir, { recursive: true });
}

// Lokasi users.json pasti di sini
const usersFile = path.join(dataDir, 'users.json');
console.log(`[DEBUG] Lokasi penyimpanan users.json (fix): ${usersFile}`);

function ensureUsersFile() {
  try {
    if (!fs.existsSync(usersFile)) {
      console.log(`[INIT] File users.json belum ada, membuat baru di: ${usersFile}`);
      fs.writeFileSync(usersFile, JSON.stringify({}, null, 2), 'utf8');
    }
    fs.accessSync(usersFile, fs.constants.R_OK | fs.constants.W_OK);
  } catch (err) {
    console.error(`❌ Gagal akses users.json (${usersFile}):`, err.message);
  }
}
...
function loadUsers() {
  ...
  try {
    ...
    console.log(`[DEBUG] loadUsers() -> ${Object.keys(data).length} user dimuat`);
    return data;
  } catch (err) {
...
function saveUsers(users) {
  ...
  try {
    ...
    console.log(`[DEBUG] saveUsers() -> sukses menyimpan ${Object.keys(users).length} user`);
  } catch (err) {
...
// ===== HANDLE TOP-UP DAN UPDATE SALDO =====
async function handleTopup(userId, amount, adminCtx) {
  ...
  try {
    let users = loadUsers();
    if (!users[userId]) {
      console.log(`[DEBUG] User ${userId} belum ada di users.json, membuat baru`);
      users[userId] = {
        saldo: 0,
        totalAkunDibuat: 0,
        role: 'User',
    ...
    users[userId].saldo = Number(users[userId].saldo || 0) + Number(amount);
    saveUsers(users);

    console.log(`[DEBUG] Saldo user ${userId} setelah top-up: Rp${users[userId].saldo.toLocaleString()}`);

    const userCtx = global.depositState[userId]?.ctx;
    if (userCtx) {
      await userCtx.reply(`✅ Top-up Rp${amount.toLocaleString()} berhasil diverifikasi!\n💳 Saldo sekarang: Rp${users[userId].saldo.toLocaleString()}`);
      await userCtx.deleteMessage(global.depositState[userId].messageId).catch(() => {});
    } else {
      console.log(`[DEBUG] Tidak bisa mengirim notifikasi ke user ${userId} (ctx hilang)`);
    }

    delete global.depositState[userId];
    await adminCtx.editMessageText(`✅ Top-up User ${userId} sebesar Rp${amount.toLocaleString()} berhasil diverifikasi.`);
  } catch (err) {
