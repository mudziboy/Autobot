...
// ===========================
// 📦 CALLBACK VIOLET MEDIA (Versi Final 21 Oktober 2025)
// ===========================
app.post("/callback", async (req, res) => {
  ...
  const ALLOWED_IPS = [
    ...
    "103.168.146.138",         // IP VPS kamu (Contoh)
      ...
      if (status === "success") {
        ...
        try {
            ...
            if (updateResult.changes === 0) {
                log(`💥 Gagal update status untuk ref ${ref}: Update 0 baris.`);
                return res.json({ status: true });
            }
            ...
            if (balanceResult.changes === 0) {
                log(`⚠️ User ${userId} tidak ditemukan di tabel users. Saldo gagal diupdate.`);
            } else {
            ...
            bot.telegram.sendMessage(userId, userMsg, { parse_mode: "Markdown" })
                .then(() => log(`📨 Notifikasi Telegram terkirim ke user ${userId}.`))
                .catch((tgErr) => log(`⚠️ Gagal kirim pesan Telegram ke user ${userId}: ${tgErr.message}`));
            ...
            if (GROUP_ID_ADMIN) {
                ...
                bot.telegram.sendMessage(GROUP_ID_ADMIN, adminMsg, { parse_mode: "Markdown" })
                    .then(() => log(`📤 Notifikasi grup admin terkirim (ID: ${GROUP_ID_ADMIN}).`))
                    .catch((err) => log(`⚠️ Gagal kirim notifikasi grup admin: ${err.message}`));
            }
            
        } catch (callbackErr) {
            log(`💥 Error callback sukses untuk ${ref}: ${callbackErr.message}. User/Saldo mungkin gagal terupdate.`);
        }
        // ============================================
        ...
        await dbRunAsync("UPDATE pending_deposits SET status = 'expired' WHERE unique_code = ?", [ref])
            .catch(err => log(`💥 Gagal update status expired untuk ${ref}: ${err.message}`));
