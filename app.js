...
// app.js (BARU, sekitar baris 39)
const vars = JSON.parse(fs.readFileSync('./.vars.json', 'utf8'));
let { // Gunakan let/var di sini jika ingin menimpa, atau definisikan ulang
  BOT_TOKEN,
  USER_ID,
  GROUP_ID,
  PORT = 50123,
...
// 🚨 KRITIS: Pastikan nilai yang akan digunakan di seluruh file sudah bersih
VIOLET_API_KEY = VIOLET_API_KEY.trim();
VIOLET_SECRET_KEY = VIOLET_SECRET_KEY.trim();
// Kini, semua referensi ke VIOLET_SECRET_KEY dan VIOLET_API_KEY
// di bawah ini akan menggunakan nilai yang sudah terjamin tanpa spasi.
...
// ===========================
// 📦 CALLBACK VIOLET MEDIA (Versi Final FIX SYNTAX)
// ===========================
app.post("/callback.php", async (req, res) => {
  ...
  // Pastikan VIOLET_API_KEY dan VIOLET_SECRET_KEY sudah di-trim di awal file (Baris 39)
  ...
  const ALLOWED_IPS = [
    ...
    "103.168.146.138",         // IP VioletMedia (Contoh/lama)
  ...
  try {
    ...
    // Pastikan req.rawBody ada. Jika kosong, ini masalah Nginx.
    const localSignature = crypto
      .createHmac("sha256", SECRET_KEY)
      .update(req.rawBody || '') // Gunakan string kosong jika rawBody undefined
      .digest("hex");
    ...
    if (isSignatureValid && isIpAllowed) {
      if (status === "success") {
        log(`✅ Callback sukses — transaksi ${ref} diverifikasi. Memulai proses Async DB.`);
        
        // === START: CALLBACK LOGIC DENGAN ASYNC/AWAIT YANG SUDAH DIPERBAIKI ===
        try {
          // 1. Ambil data transaksi dari pending_deposits
          ...
          if (!userRow) {
            ...
            // Kembalikan status sukses ke VioletMedia agar mereka tidak retry callback
            return res.json({ status: true, message: 'Transaksi tidak ditemukan' }); 
          }

          // Cegah callback duplikat
          if (String(userRow.status).toLowerCase() === "paid") {
            log(`ℹ️ Transaksi ${ref} sudah ditandai 'paid', abaikan callback duplikat.`);
            return res.json({ status: true, message: 'Sudah dibayar' });
          }

          const userId = userRow.user_id;
          const amount = Number(userRow.amount) || 0;
          const username = userRow.username || 'User'; // Gunakan username dari log atau default
          ...
          const balanceUpdateResult = await dbRunAsync(
            "UPDATE users SET saldo = COALESCE(saldo, 0) + ? WHERE user_id = ?",
            [amount, userId]
          );
          
          if (balanceUpdateResult.changes === 0) {
            log(`⚠️ User ${userId} tidak ditemukan di tabel users saat update saldo.`);
          } else {
          ...
          await dbRunAsync(`
            ...
            VALUES (?, ?, ?, ?, datetime('now')) 
          `, [userId, username, amount, ref]); 

          // 5. Kirim Notifikasi (Memanggil fungsi global)
          await sendTopupNotification(userId, amount, true); 
          log(`📨 Notifikasi dan log berhasil diproses untuk user ${userId}.`);

          // 6. Respon Sukses ke VioletMedia
          return res.json({ status: true, message: 'Success' });
          
        } catch (callbackErr) {
          log(`💥 Error FATAL saat memproses callback sukses untuk ${ref}: ${callbackErr.message}`);
          // Tetap kembalikan 200 (status: true) agar VioletMedia tidak mengulang callback
          return res.json({ status: true, message: 'Internal Server Error' }); 
        }
        // === END: CALLBACK LOGIC DENGAN ASYNC/AWAIT YANG SUDAH DIPERBAIKI ===
      ...
      // Logika jika Signature/IP Invalid
      log(`❌ Callback invalid — Signature: ${isSignatureValid}, IP: ${isIpAllowed}. Raw body length: ${req.rawBody?.length || 0}`);
      return res.status(400).json({ status: false });
    }
  } catch (err) {
    log(`💥 Error FATAL pada CALLBACK ROUTE: ${err.message}`);
...
━━━━━━━━━━━━━━━━━━━━━━━
👤 𝗨𝘀𝗲𝗿: ${mention}
📩 𝗧??𝗶𝗮𝗹 𝗯𝘆: ${roleLabel} | ${trialKe} dari ${maxTrial}
🌐 𝗦𝗲𝗿𝘃𝗲𝗿: ${namaServer}
🏪 𝗣𝗿𝗼𝘁𝗼𝗰𝗼𝗹: VMESS
⏳ 𝗗𝘂𝗿𝗮𝘀𝗶: 60 Menit
🕒 𝗪𝗮𝗸𝘁𝘂: ${new Date().toLocaleString('id-ID')}
