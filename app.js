...
bot.action('topup_saldo', async (ctx) => {
  try {
    ...
    const message = await safeSend(
        bot, 
        ctx.chat.id, 
        '💰 *Silakan scan QRIS berikut untuk melakukan top-up saldo Anda.*\n\n📌 Setelah membayar, kirim bukti (foto) dengan membalas pesan ini atau kirimkan ke chat bot ini.', 
        {
          parse_mode: 'Markdown',
          reply_markup: {
            inline_keyboard: [
    ...
    await ctx.replyWithPhoto(
      { url: qrisImageUrl }, 
      { 
        caption: 'Scan atau klik foto di atas.\nSetelah bayar, *kirim bukti (foto)* dengan membalas pesan ini atau kirim ke chat bot ini.',
        parse_mode: 'Markdown'
      }
    );
...
async function handleTopup(userId, amount, adminCtx) {
  ...
  try {
    ...
    if (!user) {
      // Jika user belum ada, insert baru dengan role 'user' default
      await dbRunAsync('INSERT INTO users (user_id, username, saldo, role, reseller_level) VALUES (?, ?, ?, ?, ?)',
        [user_id, userCtx?.from?.username || '', amount, 'user', 'silver']);
      user = { saldo: 0 };
    } else {
      // 2. Update Saldo
    ...
    if (userCtx) {
      const userMention = userCtx.from.username ? `@${userCtx.from.username}` : userCtx.from.first_name;
...
💳 *Saldo sekarang:* Rp${newSaldo.toLocaleString('id-ID')}
      ...
      // await userCtx.deleteMessage(global.depositState[userId].messageId).catch(() => {}); // Tidak perlu lagi
    }
    ...
    if (GROUP_ID) {
      const adminMention = adminCtx.from.username ? `@${adminCtx.from.username}` : adminCtx.from.first_name;
      const userMention = userCtx?.from.username ? `@${userCtx.from.username}` : userCtx?.from.first_name || `ID ${user_id}`;
...
━━━━━━━━━━━━━━━━━━━━━━━
    ...
    logger.error(`❌ handleTopup() error untuk user ${userId}:`, err);
