...
bot.action('topup_saldo', async (ctx) => {
  try {
    ...
    try {
      // Coba tampilkan gambar QRIS
      await ctx.editMessageMedia({
        type: 'photo',
        media: qrisImageUrl,
        caption: '💰 *Silakan scan QRIS berikut untuk melakukan top-up saldo Anda.*\n\n📌 Setelah membayar, kirim bukti transfer untuk verifikasi dengan menyertakan nama telegram/ID telegram | Admin : @rahmarie.',
        parse_mode: 'Markdown'
      });
    } catch (imageError) {
      // Kalau gagal kirim gambar, fallback ke pesan teks
      logger.error('⚠️ Gagal memuat gambar QRIS:', imageError);
