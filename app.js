...
// ===== Load Config (.vars.json) =====
const VARS_FILE = './.vars.json';
let vars = {};
try {
  vars = JSON.parse(fs.readFileSync(VARS_FILE, 'utf8'));
} catch (err) {
  console.warn('.vars.json tidak ditemukan, gunakan default settings');
}
...
let PORT = vars.PORT || 50123;
const PORTS = [PORT, PORT + 1, PORT + 2];
let currentPortIndex = 0;

function savePortToVars(port) {
  try {
    vars.PORT = port;
    fs.writeFileSync(VARS_FILE, JSON.stringify(vars, null, 2));
    console.log(`💾 Port ${port} tersimpan di ${VARS_FILE}`);
  } catch (err) {
    console.error('❌ Gagal menyimpan port ke .vars.json', err);
  }
}

// ===== Telegram Bot =====
const bot = new Telegraf(BOT_TOKEN || 'YOUR_BOT_TOKEN');
bot.start((ctx) => ctx.reply('🤖 Bot sellvpn siap!'));
bot.help((ctx) => ctx.reply('💡 Gunakan menu untuk kontrol bot'));

// ===== Express Route =====
app.get('/', (req, res) => res.send('Server sellvpn berjalan 🔥'));
...
function startServer() {
  ...
  server.listen(port, '::', () => {
    ...
    savePortToVars(port);
    bot.launch();
    console.log('🤖 Bot Telegram sudah jalan!');
  });

  server.on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
      console.error(`⚠️ Port ${port} sedang dipakai, coba port berikutnya...`);
      currentPortIndex++;
      if (currentPortIndex < PORTS.length) {
        startServer();
      } else {
        console.error('❌ Semua port sudah dipakai. Hentikan ghost process atau ubah manual di .vars.json');
        process.exit(1);
      }
    } else {
      console.error(err);
...
function startServer() {
  ...
  server.listen(port, '::', () => {
    ...
    savePortToVars(port); // update .vars.json
    startBot(); // jalankan bot Telegram
  });

  server.on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
      logger.error(`⚠️ Port ${port} sedang dipakai, coba port berikutnya...`);
      currentPortIndex++;
      if (currentPortIndex < PORTS.length) startServer();
      else {
        logger.error('❌ Semua port sudah dipakai. Hentikan ghost process atau ubah manual di .vars.json');
        process.exit(1);
      }
    } else {
      logger.error(err);
...
// TOPUP manual: reuse existing behavior - if manual flow expects 'topup_manual' callback_data, we'll try to route to existing handler.
// But also provide fallback to send image.png
bot.action('topup_manual', async (ctx) => {
  await ctx.answerCbQuery().catch(()=>{});
  try {
    // Try to call existing handler function if present (e.g., handleDepositState or other).
    // Fallback: send image.png in repo root.
    const img = path.join(__dirname, 'image.png');
    if (fs.existsSync(img)) {
      return ctx.replyWithPhoto({ source: img }, { caption: 'Silakan scan QRIS berikut untuk melakukan top-up. Setelah transfer, tunggu konfirmasi admin.' });
    } else {
      return ctx.reply('QRIS manual tidak tersedia. Silakan hubungi admin untuk instruksi manual.');
    }
  } catch (e) {
    console.warn('topup_manual fallback error', e && e.message);
    return ctx.reply('Terjadi kesalahan pada topup manual.');
  }
});
...
bot.action('topup_manual', async (ctx) => {
  ...
  await ctx.replyWithPhoto(qrisImageUrl, {
    caption: '💳 Silakan scan QRIS di atas untuk melakukan top up manual.\n\nKirim bukti pembayaran ke admin agar saldo diproses.'
  });
});
