...
// ===== UTILITY VIOLETMEDIA: GET CHANNEL LIST (MODIFIKASI DEBUG) =====
async function getVioletPaymentChannels() {
    try {
        ...
        logger.info(`[VIOLET] Meminta channel dari URL: ${url}`);
        // ----------------------------------------
        
        const response = await axios.post(url, postdata);
        
        // --- DEBUG LOGGING DITAMBAHKAN DI SINI ---
        logger.info(`[VIOLET] Respon Channel List: ${JSON.stringify(response.data)}`);
        // ----------------------------------------
        
        return response.data; // { status: true/false, data: [...] }
    } catch (error) {
        logger.error('❌ VIOLET API Fetch Channels Gagal total.');
        // --- DEBUG LOGGING DITAMBAHKAN DI SINI ---
        if (error.response) {
            logger.error(`❌ VIOLET Respon Server Error (HTTP ${error.response.status}): ${JSON.stringify(error.response.data)}`);
        } else {
