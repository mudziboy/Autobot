...
function createVioletSignature(ref_kode, amount) {
    const apikey = VIOLET_API_KEY;
    const secret_key = VIOLET_SECRET_KEY;
    // Format hash_hmac('sha256', ref_kode . apikey . amount, secret_key)
    const data = ref_kode + apikey + amount;
    return crypto.createHmac('sha256', secret_key).update(data).digest('hex');
}

async function requestVioletTransaction(userId, amount, email, phone, paymentMethodCode) {
    const ref_kode = `sagivpn_${userId}_${Date.now()}`;
    const signature = createVioletSignature(ref_kode, amount);
    ...
    const postdata = {
        ...
        "ref_kode": ref_kode,
        "nominal": amount,
        "cus_nama": email.split('@')[0], // Ambil bagian nama dari email jika tidak ada nama
        "cus_email": email,
        "cus_phone": phone,
        "produk": `TopUp Saldo ${NAMA_STORE} - ${amount}`,
        "url_redirect": VIOLET_CALLBACK_URL.replace('callback.php', 'redirect.php'), // Asumsi ada redirect page
        "url_callback": VIOLET_CALLBACK_URL,
        "expired_time": Date.now() / 1000 + 3600, // 1 jam expired (dalam detik)
        "signature": signature
    };
...
bot.action('topup_manual_start', async (ctx) => {
  try {
    ...
    await ctx.reply(
        '💰 *Silakan scan QRIS berikut untuk melakukan top-up saldo Anda.*\n\n📌 Setelah membayar, kirim bukti (foto) dengan membalas pesan ini atau kirimkan chat ke @rahmarie.',
        {
          parse_mode: 'Markdown'
        }
    );
    ...
    const qrisMessage = await ctx.replyWithPhoto(
      { url: qrisImageUrl },
      {
        caption: 'Scan atau klik foto di atas.\nSetelah bayar, *kirim bukti (foto)* dengan membalas pesan ke bot ini atau kirim chat ke @rahmarie.',
        parse_mode: 'Markdown'
      }
    );
...
// ===== TOP-UP OTOMATIS: PILIH CHANNEL (dari bot.on('callback_query')) =====
bot.action(/select_channel_(\w+)/, async (ctx) => {
    await ctx.answerCbQuery();
    const userId = ctx.from.id;
    const channelCode = ctx.match[1];
    ...
    await safeDeleteMessage(ctx); // Hapus pesan pilihan channel

    try {
        const amount = parseInt(depositData.amount);
        const contact = await getUserContact(userId);

        await ctx.reply('⏳ *Memproses transaksi... Mohon tunggu sebentar.*', { parse_mode: 'Markdown' });
        ...
        await safeDeleteMessage(ctx, ctx.chat.id, ctx.message.message_id + 1);

        if (response.status === true) {
            const data = response.data;
            const refId = data.ref_kode;
            const totalBayar = data.nominal_akhir;
            const expiredAt = new Date(data.expired * 1000).toLocaleString('id-ID');
            
            // Simpan data transaksi ke global state untuk polling/callback
            global.depositState[userId] = {
                action: 'await_payment_auto',
                type: 'auto',
                refId: refId,
                totalBayar: totalBayar,
                expired: data.expired * 1000
            };
            
            // Simpan transaksi di DB lokal (opsional, tapi disarankan)
            await dbRunAsync(
                `INSERT INTO pending_deposits (unique_code, user_id, amount, original_amount, timestamp, status, qr_message_id)
                 VALUES (?, ?, ?, ?, ?, ?, ?)`,
                [refId, userId, totalBayar, amount, Date.now(), 'pending_auto', 0]
            );
...
━━━━━━━━━━━━━━━━━━━━━━━
🌐 *Metode:* ${data.channel_payment}
💵 *Total Bayar:* Rp ${totalBayar.toLocaleString('id-ID')}
🕒 *Batas Bayar:* ${expiredAt}
🔖 *Ref ID:* \`${refId}\`
━━━━━━━━━━━━━━━━━━━━━━━
*Instruksi Pembayaran:* ${data.petunjuk ? data.petunjuk.replace(/<[^>]*>?/gm, '') : 'Lihat link pembayaran.'}
`.trim();
            ...
            // Jika ada QR
            if (data.qr_string) {
                // ... logic untuk mengirim QR Image jika memungkinkan ...
                await ctx.replyWithPhoto(
                    { url: `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${data.qr_string}` },
                    { caption: "Scan QRIS di atas" }
                );
            }

            // Tombol ke halaman pembayaran
            keyboard.push([{ text: '🔗 Lanjut ke Pembayaran', url: data.link_pembayaran }]);
            
            // Tombol cek status manual (opsional)
            // keyboard.push([{ text: '🔄 Cek Status Pembayaran', callback_data: `check_status_${refId}` }]); 
        ...
        logger.error('❌ Gagal proses transaksi otomatis:', e.message);
        await ctx.reply('❌ Terjadi kesalahan fatal saat memproses transaksi.', { parse_mode: 'Markdown' });
    }
});

// GANTI SELURUH BLOK app.post('/callback.php', ...) DENGAN INI:
app.post('/callback.php', async (req, res) => {
    const data = req.body;
    
    const remoteIp = req.headers['x-forwarded-for'] || req.socket.remoteAddress;

    logger.info(`[CALLBACK] VIOLETMEDIA Callback diterima dari IP: ${remoteIp}. Ref: ${data.ref}`);

    // PENTING: Membuat signature menggunakan semua komponen yang diwajibkan oleh API VioletMedia
    // Format: hash_hmac('sha256', ref_kode . apikey . nominal, secret_key)
    const requiredSignature = createVioletSignature(data.ref, data.nominal);

    if (requiredSignature !== data.signature) {
        logger.error(`❌ VIOLETMEDIA Callback gagal, signature tidak cocok untuk ref: ${data.ref}. Expected: ${requiredSignature}`);
        return res.status(400).json({ status: false, message: 'Invalid Signature' });
    }

    const refId = data.ref;
    const amountPaid = data.nominal; 
    // Ambil user ID, hati-hati jika format ref_kode berubah
    const userIdMatch = refId.match(/sagivpn_(\d+)/);
    const userId = userIdMatch ? parseInt(userIdMatch[1]) : null;

    if (!userId) {
        logger.error(`❌ VIOLETMEDIA Callback gagal, User ID tidak ditemukan di ref_kode: ${refId}`);
        return res.status(400).json({ status: false, message: 'Invalid Ref ID' });
    }

    try {
        if (data.status === 'success') {
            const row = await dbGetAsync('SELECT status FROM pending_deposits WHERE unique_code = ? AND status = "pending_auto"', [refId]);
            
            if (!row) {
                logger.warn(`⚠️ VIOLETMEDIA Callback: RefId ${refId} sudah diproses atau tidak ditemukan.`);
                return res.json({ status: true });
            }
            
            // 1. Tambah Saldo User
            await dbRunAsync('UPDATE users SET saldo = saldo + ? WHERE user_id = ?', [amountPaid, userId]);
            
            // 2. Update status di DB lokal
            await dbRunAsync('UPDATE pending_deposits SET status = "success" WHERE unique_code = ?', [refId]);

            // 3. Simpan log topup
            await dbRunAsync(`
              INSERT INTO topup_log (user_id, username, amount, reference, created_at)
              VALUES (?, ?, ?, ?, datetime('now'))
            `, [userId, data.cus_nama || 'API', amountPaid, refId]);
            
            // 4. Kirim Notifikasi (Plain Text)
            await sendTopupNotification(userId, amountPaid, true);
            
            logger.info(`✅ VIOLETMEDIA Success for user ${userId}. Amount: ${amountPaid}`);
            
            return res.json({ status: true });

        } else if (data.status === 'kadaluarsa') {
            // Hapus atau tandai sebagai expired
            await dbRunAsync('UPDATE pending_deposits SET status = "expired" WHERE unique_code = ?', [refId]);
            await sendTopupNotification(userId, amountPaid, false, 'kadaluarsa');
            return res.json({ status: true });
        } else if (data.status === 'failed') {
             await dbRunAsync('UPDATE pending_deposits SET status = "failed" WHERE unique_code = ?', [refId]);
             return res.json({ status: true });
        }
        
        await dbRunAsync('UPDATE pending_deposits SET status = ? WHERE unique_code = ?', [data.status, refId]);
        return res.json({ status: true });

    } catch (e) {
        logger.error(`❌ VIOLETMEDIA Callback Gagal Eksekusi DB untuk ${refId}: ${e.message}`);
...
async function handleAutoDepositInput(ctx, userId, data) {
    ...
    const MIN_AMOUNT = 10000; // Minimal 10.000
    ...
    if (data === 'delete') {
        ...
        if (!channelList.status || channelList.data.length === 0) {
            delete global.depositState[userId];
            return await ctx.reply('❌ Tidak ada channel pembayaran aktif saat ini.', { parse_mode: 'Markdown' });
        }

        const keyboard = channelList.data.map(c => [
            { text: `💳 ${c.name} (${c.code})`, callback_data: `select_channel_${c.code}` }
        ]);
        keyboard.push([{ text: '⬅️ Kembali', callback_data: 'topup_auto_start' }]);

        // Kirim pesan baru
        const channelMessage = await ctx.reply(`💳 *Pilih Channel Pembayaran untuk Rp ${parseInt(currentAmount).toLocaleString('id-ID')}:*`, {
            parse_mode: 'Markdown',
            reply_markup: { inline_keyboard: keyboard }
        });
        ...
        // Atur timeout baru untuk pemilihan channel
         global.depositState[userId].timeout = setTimeout(() => {
            if (global.depositState[userId] && global.depositState[userId].action === 'await_channel_select') {
                safeDeleteMessage(bot, userId, global.depositState[userId].messageId);
                delete global.depositState[userId];
                safeSend(bot, userId, '⌛ Sesi pemilihan channel expired.', { parse_mode: 'Markdown' });
            }
        }, 5 * 60 * 1000); // 5 menit
...
// ===== UTILITY VIOLETMEDIA: GET CHANNEL LIST (VERSI AMAN & EKSPLISIT) =====
async function getVioletPaymentChannels() {
    try {
        const url = `${VIOLET_API_PRODUCTION}channel-payment`;
        const postdata = {
            // PENTING: Gunakan .trim() untuk memastikan tidak ada spasi di awal/akhir key
            api_key: VIOLET_API_KEY ? VIOLET_API_KEY.trim() : '', 
            secret_key: VIOLET_SECRET_KEY ? VIOLET_SECRET_KEY.trim() : '',
            channel_payment: 'list'
        };
        ...
        logger.warn(`[VIOLET DEBUG] Payload (Key): ${postdata.api_key.substring(0, 5)}...`);
        
        // Kirim request dengan data dan konfigurasi header
