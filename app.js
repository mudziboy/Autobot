...
// Hapus semua kode dari baris ini hingga fungsi keyboard_nomor_topup(userId) terakhir.
// (Termasuk: // ===== DATABASE SQLITE =====, // ===== INIT TABEL USERS =====,
// // ===== GLOBAL STATE =====, // ===== TOP-UP USER =====, // ===== TERIMA BUKTI TRANSFER =====,
// // ===== CALLBACK ADMIN =====, // ===== HANDLE TOP-UP DAN UPDATE SALDO SQLITE =====,
// // ===== KEYBOARD NOMOR =====)
// Karena semua fungsi DB sekarang akan menggunakan 'db', 'dbGetAsync', dan 'dbRunAsync' yang sudah ada.

// 🌐 Core Modules, Express, Bot, DB, etc. (tetap di atas)
// ...

// global.depositState tetap dideklarasikan karena tidak ada di atas
if (!global.depositState) global.depositState = {};
logger.info(`Global depositState siap digunakan.`);

// ===== HANDLE CALLBACK ADMIN UTILITY =====
// PENTING: Fungsi ini akan dipanggil di dalam bot.on('callback_query', ...) di bawah.
...
const handleAdminCallback = async (ctx) => {
  ...
  if (!adminIds.includes(String(ctx.from.id))) return false;

  // === ADMIN VERIFIKASI ===
  if (data.startsWith('verif_')) {
    const userId = parts[1];
    if (!global.depositState[userId] || global.depositState[userId].action !== 'await_admin_verif') return false;
  ...
  // === ADMIN TOLAK ===
    ...
    await safeSend(bot, userCtx.chat.id, '❌ Top-up Anda *ditolak* oleh admin.', { parse_mode: 'Markdown' });
  ...
  // === TOMBOL NOMOR ===
    ...
    if (!global.depositState[userId] || global.depositState[userId].action !== 'await_admin_keyboard_nomor') return false;
...
// ===== TOP-UP USER - MANUAL ADMIN VERIFICATION (REVISI) =====
bot.action('topup_saldo', async (ctx) => {
  try {
    await ctx.answerCbQuery();
    const userId = ctx.from.id;

    logger.info(`[TOPUP] User ${userId} menekan tombol topup_saldo (Manual Mode)`);
    global.depositState[userId] = { action: 'await_bukti', timeout: null, messageId: null };

    const qrisImageUrl = 'https://raw.githubusercontent.com/mudziboy/mdzitr/refs/heads/main/qris.png';

    // Kirim pesan untuk instruksi pembayaran (agar bisa di-reply)
    const instructionMessage = await safeSend(
        bot,
        ctx.chat.id,
    ...
    // Kirim foto QRIS secara terpisah
...
// ===== TERIMA BUKTI TRANSFER - MANUAL ADMIN VERIFICATION (REVISI) =====
bot.on(['photo', 'document'], async (ctx) => {
  const userId = ctx.from.id;
  // Ambil admin ID pertama, pastikan ada
  const adminId = adminIds.length > 0 ? parseInt(adminIds[0], 10) : null; 

  if (!adminId) {
    logger.error('❌ Admin ID tidak terdefinisi di .vars.json.');
    return;
  }

  // Cek apakah user sedang dalam mode menunggu bukti top-up
  if (!global.depositState[userId] || global.depositState[userId].action !== 'await_bukti') return;
...
async function handleTopup(userId, amount, adminCtx) {
  ...
  try {
    ...
    // Tambahkan user jika belum ada. Akan diupdate saldonya di langkah selanjutnya.
    if (!user) {
    ...
    // 4. Catat Log Topup (Tabel topup_log sudah ada di atas)
    await dbRunAsync(`
      INSERT INTO topup_log (user_id, username, amount, reference, created_at)
      VALUES (?, ?, ?, ?, datetime('now'))
    `, [user_id, username, amount, `AdminVerified_${adminCtx.from.id}`, new Date().toISOString()]);

    // 5. Kirim Notifikasi Sukses ke User
    if (userCtx) {
      const successMessage = `
✅ *Top-up Rp${amount.toLocaleString('id-ID')} berhasil diverifikasi!*
👤 *Admin:* ${adminCtx.from.username ? `@${adminCtx.from.username}` : adminCtx.from.first_name}
💳 *Saldo sekarang:* Rp${newSaldo.toLocaleString('id-ID')}
      `.trim();
      await safeSend(bot, user_id, successMessage, { parse_mode: 'Markdown' });
    }
    ...
    // 7. Kirim Notifikasi ke Grup (opsional)
    if (GROUP_ID) {
      const adminMention = adminCtx.from.username ? `@${adminCtx.from.username}` : escapeMarkdown(adminCtx.from.first_name);
      const userMention = userCtx?.from.username ? `@${userCtx.from.username}` : escapeMarkdown(userCtx?.from.first_name || `ID ${user_id}`);
...
━━━━━━━━━━━━━━━━━━━━━━━
      `.trim();
      await safeSend(bot, GROUP_ID, notifGrup, { parse_mode: 'Markdown' });
    }
...
// ===== KEYBOARD NOMOR TOPUP - MANUAL ADMIN VERIFICATION (REVISI) =====
...
// GANTI KESELURUHAN FUNGSI bot.on('callback_query', async (ctx) => { ... } INI
...
bot.on('callback_query', async (ctx) => {
  ...
  // === HANDLE MANUAL TOPUP ADMIN CALLBACKS (DIPRIORITASKAN) ===
  // Cek apakah ini adalah callback verifikasi top-up admin (verif_, tolak_, atau keyboard input nominal)
  ...
  // 1️⃣ HANDLE DEPOSIT (Logika Lama/Otomatis QRIS - jika masih ada)
  ...
  // 3️⃣ HANDLE INLINE ADMIN TOOLS (LANJUTAN)
  ...
  if (data === 'admin_backup_db') {
    ...
    const files = fs.readdirSync(BACKUP_DIR)
      ...
      .sort()
      .slice(-10); // Ambil 10 file terbaru

    await ctx.reply(`✅ *Backup berhasil:* \`${backupFileName}\`\n\n${files.length} file backup tersimpan.`, {
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [
          [{ text: '🔽 Restore DB', callback_data: 'admin_restore_db' }]
        ]
      }
    });
    logger.info(`[BACKUP] ${ctx.from.username} created backup: ${backupFileName}`);
  }
  ...
  if (data === 'admin_restore_db') {
    const files = fs.readdirSync(BACKUP_DIR)
      ...
      .slice(0, 5); // Hanya tampilkan 5 file terbaru

    if (files.length === 0) {
      return ctx.reply('❌ *Tidak ada file backup yang ditemukan.*', { parse_mode: 'Markdown' });
    }

    const keyboard = files.map(file => [{
      text: `${file} (${fs.statSync(path.join(BACKUP_DIR, file)).size / (1024 * 1024) | 0}MB)`,
      callback_data: `restore_file::${file}`
    }]);

    await ctx.editMessageText('🔽 *Pilih file backup untuk restore:*\n\n⚠️ *PERINGATAN:* Proses ini akan mengganti database saat ini.', {
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: keyboard
      }
  ...
  if (data.startsWith('restore_file::')) {
    ...
    try {
      ...
      fs.copyFileSync(filePath, DB_PATH);
      await ctx.editMessageText(`✅ *Restore berhasil dari:* \`${fileName}\`\n\n*Bot mungkin perlu di-restart untuk menerapkan perubahan.*`, { parse_mode: 'Markdown' });
...
}); // <-- INI ADALAH PENUTUP KURUNG KURAWAL YANG BENAR UNTUK bot.on('callback_query', ...)
