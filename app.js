  ...
  try {
    ...
    if (isSignatureValid && isIpAllowed) {
      if (status === "success") {
        ...
        // TODO: tambahkan logika update saldo / status transaksi di sini
        // ✅ [NEW LOGIC: Update saldo user & kirim notifikasi Telegram]
// =============================================================
try {
  ...
  db.get(
    ...
    async (err, row) => {
      ...
      if (row.status === "paid") {
        log(`ℹ️ Transaksi ${ref} sudah pernah ditandai sebagai paid, abaikan callback duplikat.`);
        return;
      }
      ...
      // 🔹 Tambahkan saldo user di tabel users
      db.run(
  ...
  log(`💥 Error saat proses callback sukses untuk ${ref}: ${callbackErr.message}`);
}
