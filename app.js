...
// PERBAIKI FUNGSI createVioletSignature
function createVioletSignature(ref_kode, amount) {
    const apikey = VIOLET_API_KEY.trim();
    const secret_key = VIOLET_SECRET_KEY.trim();
    // PENTING: Pastikan amount di-string-ify jika belum
    const data = ref_kode + apikey + String(amount); 
    return crypto.createHmac('sha256', secret_key).update(data).digest('hex');
}

// GANTI FUNGSI requestVioletTransaction DENGAN VERSI DEBUG
...
async function requestVioletTransaction(userId, amount, email, phone, paymentMethodCode) {
    ...
    // ... (append semua formData, sama seperti sebelumnya) ...
    // ...
    formData.append("signature", signature); // Pastikan signature ada di akhir
...
async function handleAutoDepositInput(ctx, userId, data) {
    ...
    if (data === 'delete') {
        ...
        // GANTI BLOK INI di dalam handleAutoDepositInput() (setelah channelList = await getVioletPaymentChannels();)
        ...
        const allowedChannels = ['QRIS']; // Tambahkan kode channel lain di sini jika ingin memunculkan lebih banyak
        
        const filteredChannels = channelList.data
            .filter(c => c.status === 'Aktif' && allowedChannels.includes(c.nama)); 
        // --- FILTER KODE BARU SELESAI DI SINI ---

        if (!channelList.status || filteredChannels.length === 0) { // <-- Ganti channelList.data dengan filteredChannels
            delete global.depositState[userId];
            // Tambahkan notifikasi jika semua channel yang difilter tidak tersedia
            return await ctx.reply('❌ Maaf, metode pembayaran QRIS sedang tidak aktif saat ini.', { parse_mode: 'Markdown' });
        }

        // --- Ganti mapping keyboard di bawah dengan filteredChannels ---
        const keyboard = filteredChannels.map(c => [ // <-- GUNAKAN filteredChannels
            // PENTING: Callback data harus menggunakan properti yang unik, biasanya 'code'
            { text: `💳 ${c.nama} - Biaya: ${c.biaya}%`, callback_data: `select_channel_${c.code}` } 
        ]);
