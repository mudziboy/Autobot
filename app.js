...
💵 Harga /hari:
   • Non-reseller : Rp${hargaNon.toLocaleString('id-ID')}
   • Reseller     : Rp${hargaReseller.toLocaleString('id-ID')}
   • Gold         : Rp${hargaGold.toLocaleString('id-ID')}
   • Platinum     : Rp${hargaPlatinum.toLocaleString('id-ID')}

💳 Harga /bulan:
   • Non-reseller : Rp${hargaNon30.toLocaleString('id-ID')}
   • Reseller     : Rp${hargaReseller30.toLocaleString('id-ID')}
   • Gold         : Rp${hargaGold30.toLocaleString('id-ID')}
   • Platinum     : Rp${hargaPlatinum30.toLocaleString('id-ID')}
  ...
  try {
    ...
    try {
      ...
      await ctx.editMessageMedia({
        type: 'photo',
        media: qrisImageUrl,
        caption: '💰 *Silakan scan QRIS berikut untuk melakukan top-up saldo Anda.*\n\n📌 Setelah membayar, kirim bukti transfer untuk verifikasi dengan menyertakan nama telegram/ID telegram | Admin : @sagistore_id.',
        parse_mode: 'Markdown'
      });
    } catch (imageError) {
    ...
    await ctx.editMessageText(
      '❌ *GAGAL! Terjadi kesalahan saat memproses permintaan Anda. Silakan coba lagi nanti atau hubungi Admin @sagistore_id.*',
      { parse_mode: 'Markdown' }
    );
  }
});
...
bot.action(/add_saldo_(\d+)/, async (ctx) => {
  ...
  // Kirim pesan, tanpa inline keyboard
