...
// === CONFIG ===
const BOT_TOKEN = '8494066951:AAF8LH4Gq1Q_ctDl86IrXLXXnUk_3oO7zJY';
const ADMIN_CHAT_ID = 8372210994; // ganti dengan chat id admin
const QRIS_URL = 'https://raw.githubusercontent.com/mudziboy/mdzitr/refs/heads/main/qris.png';
...
// Buat tabel user dan topup jika belum ada
db.serialize(() => {
  db.run(`CREATE TABLE IF NOT EXISTS users (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
...
// Helper function tambah saldo
function addUserSaldo(telegramId, nominal) {
  db.run(`INSERT INTO users(telegram_id, saldo) VALUES(?, ?) 
          ON CONFLICT(telegram_id) DO UPDATE SET saldo = saldo + ?`,
    [telegramId, nominal, nominal]
  );
}
...
// Step 1: User klik topup
bot.action('topup_saldo', async (ctx) => {
  const userId = ctx.from.id;
  await ctx.answerCbQuery();

  depositState[userId] = { action: 'request_amount', amount: 0 };
  await ctx.editMessageMedia({
    type: 'photo',
    media: QRIS_URL,
    caption: '💰 Silakan scan QRIS berikut untuk melakukan top-up saldo.\n\n📌 Setelah membayar, kirim bukti transfer untuk verifikasi.',
  });
});

// Step 2: User kirim nominal (opsional)
bot.on('text', async (ctx) => {
  const userId = ctx.from.id;
  ...
  if (depositState[userId] && depositState[userId].action === 'request_amount') {
    ...
    if (isNaN(nominal) || nominal <= 0) {
      await ctx.reply('❌ Masukkan nominal yang valid!');
      return;
    }
    depositState[userId].amount = nominal;
    depositState[userId].action = 'waiting_bukti';
    await ctx.reply('📌 Nominal diterima. Silakan kirim bukti transfer (foto).');
  }
});

// Step 3: User upload bukti transfer
bot.on('photo', async (ctx) => {
  const userId = ctx.from.id;
  if (depositState[userId] && depositState[userId].action === 'waiting_bukti') {
    const photoArray = ctx.message.photo;
    const fileId = photoArray[photoArray.length - 1].file_id;

    // Simpan ke DB
    const nominal = depositState[userId].amount;
    db.run(`INSERT INTO topup_requests(user_id, nominal, status, bukti_file_id)
            VALUES(?, ?, ?, ?)`,
      [userId, nominal, 'waiting_approval', fileId]
    );

    depositState[userId].action = 'waiting_approval';

    // Kirim notifikasi ke admin
    await ctx.telegram.sendPhoto(
      ADMIN_CHAT_ID,
      fileId,
      { caption: `User @${ctx.from.username || userId} melakukan topup ${nominal}\nKlik /approve_${userId} atau /reject_${userId}` }
    );

    await ctx.reply('📌 Bukti transfer diterima. Menunggu verifikasi admin...');
  }
});
...
bot.command(/approve_(\d+)/, async (ctx) => {
  ...
  // Ambil request topup terakhir yang waiting_approval
  db.get(`SELECT id, nominal FROM topup_requests WHERE user_id = ? AND status = 'waiting_approval' ORDER BY created_at DESC LIMIT 1`, [userId], async (err, row) => {
    if (err || !row) {
      await ctx.reply('⚠️ Tidak ada topup yang menunggu approval untuk user ini.');
      return;
    }

    // Update status
    db.run(`UPDATE topup_requests SET status = 'approved' WHERE id = ?`, [row.id]);
    addUserSaldo(userId, row.nominal);

    // Notify user
    await ctx.telegram.sendMessage(userId, `✅ Top-up berhasil! Saldo Anda bertambah ${row.nominal}.`);
    await ctx.reply(`User ${userId} topup berhasil.`);
  });
});
...
bot.command(/reject_(\d+)/, async (ctx) => {
  ...
  db.get(`SELECT id FROM topup_requests WHERE user_id = ? AND status = 'waiting_approval' ORDER BY created_at DESC LIMIT 1`, [userId], async (err, row) => {
    if (err || !row) {
      await ctx.reply('⚠️ Tidak ada topup yang menunggu approval untuk user ini.');
      return;
    }

    db.run(`UPDATE topup_requests SET status = 'rejected' WHERE id = ?`, [row.id]);
    await ctx.telegram.sendMessage(userId, '❌ Top-up Anda ditolak. Silakan hubungi admin.');
    await ctx.reply(`User ${userId} topup ditolak.`);
  });
});
