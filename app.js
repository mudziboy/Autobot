...
bot.action('topup_saldo', async (ctx) => {
  try {
    ...
    logger.error('❌ Kesalahan saat memulai top-up:', error);
...
bot.on('callback_query', async (ctx) => {
  ...
  if (data.startsWith('verif_')) {
    ...
    // Mulai input nominal via inline keyboard khusus top-up user
  ...
  // ===== TOMBOL NOMOR / OK / CLEAR =====
    ...
    // Perbaikan split: keyboard_{key}_{userId}
...
async function handleTopup(userId, amount, adminCtx) {
  ...
  if (!users[userId]) {
    users[userId] = { saldo: 0, totalAkunDibuat: 0, role: 'User', username: '', createdAt: new Date().toISOString() };
  }

  // Pastikan saldo number
  users[userId].saldo = Number(users[userId].saldo || 0) + Number(amount);
  saveUsers(users);

  console.log(`✅ Saldo user ${userId} setelah top-up: ${users[userId].saldo}`);
