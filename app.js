...
app.post('/callback.php', async (req, res) => {
    ...
    // 🚨 KOREKSI KRUSIAL: Ambil signature dari header 'X-Callback-Signature'
    const sentSignature = req.headers['x-callback-signature']; // Headers di Express/Node.js otomatis lower-case

    const remoteIp = req.headers['x-forwarded-for'] || req.socket.remoteAddress;

    logger.info(`[CALLBACK] VIOLETMEDIA Callback diterima dari IP: ${remoteIp}. Ref: ${data.ref}, API Status: ${data.status}. Signature Diterima: ${sentSignature ? 'YA' : 'TIDAK'}`);

    // app.js (Sekitar Baris 5644 - Di dalam app.post('/callback.php', ...))

    // PENTING: Panggilan Signature untuk CALLBACK
    ...
    try {
        ...
        if (!row) {
            logger.warn(`⚠️ VIOLETMEDIA Callback: RefId ${refId} tidak ditemukan di pending_deposits.`);
        ...
        if (data.status === 'success') {
            ...
            // 1. Tambah Saldo User (Menggunakan amountPaid yang sudah berupa angka)
