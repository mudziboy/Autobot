...
// GANTI SELURUH BLOK bot.action(/select_channel_(\w+)/) DENGAN KODE INI

bot.action(/select_channel_(\w+)/, async (ctx) => {
    await ctx.answerCbQuery();
    const userId = ctx.from.id;
    const channelName = ctx.match[1]; // channelName = 'QRIS'
    ...
    // Hapus pesan channel selection (menggunakan ctx yang asli)
    await safeDeleteMessage(ctx); 
    ...
    try {
        ...
        // 🎯 LOGIKA PERBAIKAN: Gunakan kode internal yang diduga (QRISC)
        let paymentCodeToSend = channelName; 

        if (channelName === 'QRIS') {
            // Coba kode internal yang diduga benar oleh API
            paymentCodeToSend = 'QRISD'; 
        }

        const processingMessage = await ctx.reply('⏳ *Memproses transaksi... Mohon tunggu sebentar.*', { parse_mode: 'Markdown' });
        
        // Panggil API dengan kode internal yang diuji coba
        const response = await requestVioletTransaction(
            userId, 
            amount, 
            contact.email, 
            contact.phone, 
            paymentCodeToSend // Menggunakan 'QRISC'
        );
        ...
        if (response.status === true) {
            // PASTIKAN AKSES KE DATA DENGAN BENAR: response.data.data
            ...
            // Simpan data transaksi ke global state
            global.depositState[userId] = {
            ...
            await dbRunAsync(
                `INSERT INTO pending_deposits (unique_code, user_id, amount, original_amount, timestamp, status, qr_message_id)
                 VALUES (?, ?, ?, ?, ?, ?, ?)`,
                [refId, userId, totalBayar, amount, DateAt.now(), 'pending_auto', 0]
            );
...
`.trim();
            ...
            // Jika ada QR, kirim gambar
            if (data.qr_string) {
                const qrLink = `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(data.qr_string)}`;
...
async function handleAutoDepositInput(ctx, userId, data) {
    ...
    // GANTI BLOK INI di dalam handleAutoDepositInput() (mapping keyboard)

        // --- Ganti mapping keyboard di bawah dengan filteredChannels ---
        const keyboard = filteredChannels.map(c => [
            // PENTING: Gunakan 'nama' sebagai callback_data
            { text: `💳 ${c.nama} - Biaya: ${c.biaya}%`, callback_data: `select_channel_${c.nama}` } 
        ]);
