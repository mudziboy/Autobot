...
📜 *Akun Dibuat* : ${escapeMarkdownV2(String(totalAkunDibuat))}
🏷 *Status* : ${escapeMarkdownV2(roleLabel)}
🆔 *ID Anda* : \`${userId}\` // <-- Dibiarkan tanpa escapeMarkdownV2 untuk backtick, tapi pastikan userId berupa string
🔒 *Admin Bot* : @${escapeMarkdownV2('sagistore_id')}
...
// ===== TOP-UP USER - MANUAL ADMIN VERIFICATION =====
bot.action('topup_saldo', async (ctx) => {
  try {
    // Gunakan dbRunAsync / dbGetAsync yang dideklarasikan di atas
    ...
    // Periksa apakah pesan perlu diedit atau dikirim baru
    const message = await safeSend(
        bot, 
        ctx.chat.id, 
        '💰 *Silakan scan QRIS berikut untuk melakukan top-up saldo Anda.*\n\n📌 Setelah membayar, jangan lupa untuk membalas pesan ini dan sertakan foto bukti transfer.', 
        {
          parse_mode: 'Markdown',
          reply_markup: {
            inline_keyboard: [
    ...
    // Karena kita tidak mengedit pesan untuk kirim foto QRIS (agar user bisa reply), kita kirim pesan terpisah dengan foto.
    // Kirim foto QRIS secara terpisah agar user bisa reply ke pesan teks di atas.
    await ctx.replyWithPhoto(
      { url: qrisImageUrl }, 
      { 
        caption: 'Scan atau klik foto di atas.\nSetelah bayar, *kirim bukti (foto)* transfer dengan membalas pesan bot ini.',
        parse_mode: 'Markdown'
      }
    );


    // global.depositState[userId].messageId = message.message_id; // Tidak perlu, karena prosesnya manual via reply
    ...
    global.depositState[userId].timeout = setTimeout(async () => {
      if (global.depositState[userId] && global.depositState[userId].action === 'await_bukti') {
        // await ctx.deleteMessage(global.depositState[userId].messageId).catch(() => {});
...
// ===== TERIMA BUKTI TRANSFER - MANUAL ADMIN VERIFICATION =====
bot.on(['photo', 'document'], async (ctx) => {
  const userId = ctx.from.id;
  const adminId = parseInt(adminIds[0], 10); // Ambil admin ID pertama dari list di atas
...
// GANTI SELURUH FUNGSI handleTopup DENGAN VERSI INI:
...
// ===== HANDLE TOP-UP DAN UPDATE SALDO SQLITE (REVISI) - BLOK 7 =====
// ...
// ... (di dalam fungsi handleTopup) ...

// 7. Kirim Notifikasi ke Grup (opsional)
if (typeof GROUP_ID !== 'undefined' && GROUP_ID) {
  // Pastikan Anda menggunakan fungsi escape yang paling ketat (misalnya escapeMarkdownV2 jika ada, atau escapeMarkdown)
  const adminMention = adminCtx.from.username ? `@${escapeMarkdownV2(adminCtx.from.username)}` : escapeMarkdownV2(adminCtx.from.first_name);
  const userMention = userCtx?.from.username ? `@${escapeMarkdownV2(userCtx.from.username)}` : escapeMarkdownV2(userCtx?.from.first_name || `ID ${user_id}`);
...
━━━━━━━━━━━━━━━━━━━━━━━
  ...
  // Masalahnya mungkin terjadi di sini: Jika safeSend tidak otomatis menggunakan escapeMarkdownV2.
  // Ganti `parse_mode: 'Markdown'` menjadi `'MarkdownV2'` jika Anda yakin bot Anda default ke V2.
  await safeSend(bot, GROUP_ID, notifGrup, { parse_mode: 'MarkdownV2' });
}
    // 8. Hapus State
    delete global.depositState[userId];
