...
// Berdasarkan dokumentasi API, signature dibuat hanya dari ref_kode dan SECRET_KEY (sebagai key HMAC)
function createVioletSignature(ref_kode) { 
    const secret_key = VIOLET_SECRET_KEY.trim();
    // Data yang di-hash adalah ref_kode
    const data = ref_kode; 
...
app.post('/callback.php', async (req, res) => {
    ...
    // PENTING: Panggilan Signature hanya membutuhkan data.ref
    const requiredSignature = createVioletSignature(data.ref); 
