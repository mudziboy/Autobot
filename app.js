...
// ===== Graceful Shutdown =====
async function safeStopBot() {
  if (bot && botLaunched) {
    try {
      await bot.stop();
      console.log('✅ Bot berhenti dengan aman');
    } catch (err) {
      console.warn('⚠️ Bot gagal berhenti:', err.message);
    }
  } else {
    console.log('ℹ️ Bot belum berjalan, skip stop');
  }
  process.exit();
}

process.once('SIGINT', safeStopBot);
process.once('SIGTERM', safeStopBot);
