// ===== DEBUG LOG =====
const DEBUG_LOG = true; // debug logging ringan
function dlog(msg) {
  if (typeof DEBUG_LOG !== 'undefined' && DEBUG_LOG) console.log('[DEBUG]', msg);
}
...
// Safe pm2 restart wrapper (injected automatically)
function safePm2Restart(name) {
  try {
    exec('pm2 restart ' + name, (err, stdout, stderr) => {
      if (err) {
        console.error('❌ Gagal restart via PM2: ' + (err.message || err));
      } else {
        console.log('✅ pm2 restart ' + name + ' executed');
      }
    });
  } catch (e) {
    console.error('safePm2Restart exception: ' + e.message);
  }
}

// Database & Paths
const DB_PATH = '/root/BotVPN2/sellvpn.db';
...
// Load .vars.json dengan aman
let vars = {};
try {
  vars = JSON.parse(fs.readFileSync('/root/BotVPN2/.vars.json', 'utf8'));
  if (!vars.BOT_TOKEN) throw new Error('BOT_TOKEN tidak ditemukan di .vars.json');
} catch (e) {
  console.error('❌ Gagal membaca .vars.json atau BOT_TOKEN tidak ada:', e.message);
  process.exit(1);
}
...
// Debug log
dlog('Bot berhasil diinisialisasi dengan BOT_TOKEN dari .vars.json');

// 📦 Tools
const { promisify } = require('util');
// QRISPayment removed — use Violet Media integration
...
// 🧠 Admin List
const rawAdmin = vars.ADMIN_ID; // ambil dari .vars.json
...
cron.schedule('0 4 * * *', () => {
  logger.warn('🌀 Restart harian bot');
  logger.info('bot.restart() removed — use pm2 restart');
});
...
bot.action('confirm_upgrade_reseller', async (ctx) => {
  ...
  await ctx.reply(
    escapeMarkdownV2(
      ...
      'Silakan mulai transaksi dengan harga lebih murah! (harga yang ditampilkan dimenu adalah harga member)'
    ),
    { parse_mode: 'MarkdownV2' }
  );
...
🏪 𝗣𝗿𝗼𝘁𝗼𝗰𝗼𝗹: SSH
⏳ 𝗗𝘂𝗿𝗮𝘀𝗶: 60 Menit
🕒 𝗪𝗮??𝘁𝘂: ${new Date().toLocaleString('id-ID')}
━━━━━━━━━━━━━━━━━━━━━━━
          `.trim();
...
bot.action('topup_saldo', async (ctx) => {
  // === Reworked Top Up Saldo menu ===
  await ctx.answerCbQuery().catch(() => {});
  const keyboard = {
    inline_keyboard: [
      [{ text: '💳 Top Up Manual (QRIS Lama)', callback_data: 'topup_manual' }],
      [{ text: '⚡ Top Up Otomatis (Violet Media)', callback_data: 'topup_otomatis' }],
      [{ text: '⬅️ Kembali ke Menu Utama', callback_data: 'send_main_menu' }]
    ]
  };
  await ctx.editMessageText('💰 *Pilih metode top up saldo Anda:*', {
    parse_mode: 'Markdown',
    reply_markup: keyboard
  });
  return;
...
bot.on('callback_query', async (ctx) => {
  ...
  // --- Violet Media Top-Up: show dual mode menu (manual vs otomatis) ---
  try {
    const cbdata = ctx.callbackQuery && ctx.callbackQuery.data;
    if (cbdata === 'topup_saldo') {
      // reply with two choices: manual (existing flow) or otomatis (Violet)
      const keyboard = {
        inline_keyboard: [
          [{ text: '1️⃣ Top Up Manual (QRIS)', callback_data: 'topup_manual' }],
          [{ text: '2️⃣ Top Up Otomatis (Violet)', callback_data: 'topup_otomatis' }],
          [{ text: '❌ Batal', callback_data: 'topup_cancel' }]
        ]
      };
      try {
        // edit message markup if possible else send new message
        if (ctx.update.callback_query && ctx.update.callback_query.message) {
          await ctx.editMessageReplyMarkup(keyboard);
        } else {
          await ctx.reply('Pilih metode top-up:', { reply_markup: keyboard });
        }
      } catch (e) {
        await ctx.reply('Pilih metode top-up:', { reply_markup: keyboard });
      }
      return; // stop further processing of this callback
    }
  } catch (e) {
    console.warn('Violet topup menu injection error:', e && e.message);
  }
  // --- end injection ---

 // selalu akhiri loading tombol
...
app.listen(PORT, () => {
  ...
  // 🟢 Tambahkan ini di bawah fungsi untuk memulai polling bot
  startBot();
});
// 🟢 Tambahkan ini di luar app.listen() untuk jaga-jaga polling tetap hidup
bot.launch()
  .then(() => console.log('🤖 Bot started in polling mode (fallback)'))
  .catch(err => console.error('❌ Gagal memulai bot fallback:', err));

/* --- BEGIN Violet Media Top-Up Integration (ADDED) --- */

// per-user state for violet automatic deposit flow (keeps separate from existing depositState)
const violetDepositState = {}; // violetDepositState[userId] = { action: 'await_amount', ref_kode }

// helper: generate short unique ref
...
// helper: create signature for create API: HMAC-SHA256(ref_kode + apikey + amount, secret_key)
function createCreateSignature(ref_kode, apikey, amount, secret_key) {
  try {
    return require('crypto').createHmac('sha256', String(secret_key)).update(String(ref_kode) + String(apikey) + String(amount)).digest('hex');
  } catch (e) {
    return '';
  }
}

// wrapper: create Violet transaction
async function createVioletTransaction({ amount, method = 'QRIS', ref_kode, cus_nama = 'User', cus_email = '', cus_phone = '', produk = 'TopUp', url_redirect = '', url_callback = '' }) {
  const axios = require('axios');
  const apikey = (typeof VIOLET_APIKEY !== 'undefined') ? VIOLET_APIKEY : (vars && vars.VIOLET_APIKEY);
  const secret = (typeof VIOLET_SECRET !== 'undefined') ? VIOLET_SECRET : (vars && vars.VIOLET_SECRET);
  const base = (typeof VIOLET_API_URL !== 'undefined') ? VIOLET_API_URL : (vars && vars.VIOLET_API_URL) || 'https://violetmediapay.com/api/live';
  if (!apikey || !secret) throw new Error('Violet API key/secret tidak dikonfigurasi');

  if (!ref_kode) ref_kode = generateRef();
  const signature = createCreateSignature(ref_kode, apikey, String(amount), secret);

  const postData = new URLSearchParams();
  postData.append('api_key', apikey);
  postData.append('secret_key', secret);
  ...
  const res = await axios.post(base + '/create', postData.toString(), { headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, timeout: 20000 });
  return res.data;
}

const qrisImageUrl = 'https://raw.githubusercontent.com/mudziboy/mdzitr/refs/heads/main/qris.png';
const ADMIN_ID = 8372210994; // ganti dengan telegram ID admin
...
bot.action('topup_manual', async (ctx) => {
  try {
    ...
    logger.info(`🔍 User ${userId} memulai proses TOPUP MANUAL.`);

    // Kirim QRIS ke user
    let qrMessage;
    try {
      qrMessage = await ctx.replyWithPhoto(qrisImageUrl, {
        caption: '💳 *Silakan scan QRIS berikut untuk melakukan top-up manual.*\n\n📌 Setelah membayar, kirim bukti transfer di chat ini untuk verifikasi admin. QRIS akan expired dalam 5 menit.',
        parse_mode: 'Markdown'
      });
    } catch (imageError) {
      logger.error('⚠️ Gagal memuat gambar QRIS topup_manual:', imageError);
      return await ctx.reply(
        '⚠️ *Gagal memuat gambar QRIS.*\nSilakan hubungi admin untuk mendapatkan QRIS terbaru:\n👉 [t.me/rahmarie](https://t.me/rahmarie)',
        { parse_mode: 'Markdown' }
      );
    }
    ...
    // Timer 5 menit untuk auto-expire
    global.depositState[userId].timer = setTimeout(async () => {
      if (!global.depositState[userId]) return;

      try {
        // Hapus pesan QRIS
        await ctx.deleteMessage(global.depositState[userId].qrMessageId).catch(()=>{});
        await ctx.reply('⏰ QRIS top-up manual Anda telah expired karena tidak ada tindakan dalam 5 menit.');
      } catch (err) {
        logger.error('❌ Error auto-expire QRIS:', err);
      }
    ...
    logger.error('❌ Kesalahan saat memproses topup_manual:', error);
    await ctx.reply('❌ Terjadi kesalahan saat memproses topup manual. Silakan coba lagi nanti.');
  }
});

// === MENANGANI NOMINAL TOPUP ===
bot.on('text', async (ctx) => {
  const userId = ctx.from.id;
  const state = global.depositState[userId];
  if (!state || state.action !== 'request_amount') return;

  const amount = parseInt(ctx.message.text.replace(/\D/g, ''));
  if (isNaN(amount) || amount < 10000) {
    return ctx.reply('⚠️ Minimal top up adalah Rp10.000. Silakan masukkan jumlah yang valid.');
  }

  state.amount = amount;
  state.action = 'await_proof';

  await ctx.reply(`✅ Nominal top-up manual Anda: Rp${amount.toLocaleString()}\n📌 Silakan transfer sesuai nominal dan kirim bukti transfer di chat ini untuk verifikasi admin.`);
});

// === MENANGANI BUKTI TRANSFER ===
bot.on(['photo', 'document'], async (ctx) => {
  ...
  if (ctx.message.photo) {
    ...
    return ctx.reply('⚠️ Harap kirimkan foto atau dokumen sebagai bukti transfer.');
  }
  ...
  await ctx.telegram.sendMessage(
    ADMIN_ID,
    `📌 Bukti transfer dari user @${ctx.from.username || ctx.from.first_name} (${userId}) untuk topup manual Rp${state.amount.toLocaleString()} telah dikirim.\nGunakan /verifikasi_manual ${userId} untuk menambahkan saldo.`
  );

  await ctx.reply('✅ Bukti transfer berhasil dikirim ke admin. Mohon tunggu konfirmasi.');
});

// === COMMAND ADMIN UNTUK VERIFIKASI ===
bot.command('verifikasi_manual', async (ctx) => {
  if (ctx.from.id !== ADMIN_ID) return;

  const args = ctx.message.text.split(' ');
  const userId = parseInt(args[1]);
  if (!userId || !global.depositState[userId] || global.depositState[userId].action !== 'await_proof') {
    return ctx.reply('❌ User tidak ditemukan atau belum mengirim bukti.');
  }

  const state = global.depositState[userId];

  // Tambahkan saldo user (sesuaikan implementasi sesuai database)
  // await addBalance(userId, state.amount);

  state.action = 'done';

  // Hapus timer jika masih berjalan
  if (state.timer) clearTimeout(state.timer);

  // Hapus QRIS message di chat user
  try {
    await ctx.telegram.deleteMessage(userId, state.qrMessageId).catch(()=>{});
  } catch (err) {
    logger.error('❌ Gagal menghapus QRIS message setelah verifikasi:', err);
  }

  // Konfirmasi ke user
  try {
    await ctx.telegram.sendMessage(userId, `✅ Top-up manual Anda sebesar Rp${state.amount.toLocaleString()} telah diverifikasi dan saldo masuk.`);
  } catch (err) {
    logger.error('❌ Gagal mengirim konfirmasi ke user:', err);
  }

  // Konfirmasi ke admin
  await ctx.reply(`✅ Top-up manual untuk user ${userId} sebesar Rp${state.amount.toLocaleString()} berhasil diverifikasi.`);

  // Hapus state
  delete global.depositState[userId];
});

// === TOP UP OTOMATIS (Violet Media) ===
bot.action('topup_otomatis', async (ctx) => {
  await ctx.answerCbQuery().catch(() => {});
  const userId = ctx.from.id;
  violetDepositState[userId] = { action: 'await_amount' };
  await ctx.reply('💰 Masukkan nominal top-up yang ingin Anda isi (minimal Rp10.000):');
});

bot.on('text', async (ctx) => {
  const userId = ctx.from.id;
  const state = violetDepositState[userId];
  if (!state || state.action !== 'await_amount') return;
  const amount = parseInt(ctx.message.text.replace(/\D/g, ''));
  if (isNaN(amount) || amount < 10000) {
    return ctx.reply('⚠️ Minimal top up adalah Rp10.000. Silakan masukkan jumlah yang valid.');
  }
  const ref = generateRef();
  const callbackUrl = `https://${vars.DOMAIN}/callback.php`;
  try {
    const trx = await createVioletTransaction({
      amount,
      ref_kode: ref,
      cus_nama: ctx.from.first_name,
      url_callback: callbackUrl,
      url_redirect: `https://${vars.DOMAIN}`
    });
    violetDepositState[userId] = null;
    if (trx?.data?.payment_url) {
      await ctx.reply(`✅ Transaksi berhasil dibuat!\n\nSilakan lakukan pembayaran melalui link berikut:\n${trx.data.payment_url}`);
    } else {
      await ctx.reply('❌ Gagal membuat transaksi. Coba lagi nanti.');
    }
  } catch (err) {
    console.error('createVioletTransaction error:', err);
    await ctx.reply('❌ Gagal membuat transaksi di payment gateway. Silakan coba lagi nanti.');
  }
});

bot.action('topup_cancel', async (ctx) => {
  await ctx.answerCbQuery().catch(()=>{});
  try { await ctx.editMessageReplyMarkup(null); } catch(e) {}
  const uid = String(ctx.from.id);
  delete violetDepositState[uid];
  await ctx.reply('Batal top-up.');
});

// capture text input for automatic topup amount
bot.on('text', async (ctx, next) => {
  try {
    const uid = String(ctx.from.id);
    const state = violetDepositState[uid];
    if (!state || state.action !== 'await_amount') return next();
    const raw = String(ctx.message.text || '').replace(/[^\d]/g, '');
    const amount = parseInt(raw || '0', 10);
    if (!amount || amount < 10000) {
      await ctx.reply('Minimal top up adalah Rp10.000. Silakan ketik ulang nominal yang benar.');
      return;
    }

    // create transaction: save pending_deposits if DB has table pending_deposits
    const ref_kode = generateRef();
    const url_callback = `https://${(typeof DOMAIN !== 'undefined' ? DOMAIN : (vars && vars.DOMAIN) || 'sagivpn.my.id')}/callback.php`;

    // try insert into DB if possible (table name may vary: try pending_deposits or pending_deposit)
    try {
      await dbRunAsync('INSERT OR IGNORE INTO pending_deposits (unique_code, user_id, amount, original_amount, status) VALUES (?, ?, ?, ?, ?)', [ref_kode, uid, amount, amount, 'pending']);
    } catch (e) {
      // ignore DB error but log
      console.warn('Failed insert pending_deposits', e && e.message);
    }

    await ctx.reply('Membuat transaksi... Mohon tunggu.');

    let createResp;
    try {
      createResp = await createVioletTransaction({
        amount,
        method: 'QRIS',
        ref_kode,
        cus_nama: ctx.from.first_name || ctx.from.username || `User_${uid}`,
        cus_email: '',
        cus_phone: '',
        produk: 'TopUp Saldo',
        url_redirect: '',
      ...
      console.error('createVioletTransaction error', err && err.message || err);
      delete violetDepositState[uid];
      return ctx.reply('Gagal membuat transaksi di payment gateway. Silakan coba lagi nanti atau pilih top-up manual.');
    }

    // store response in DB if possible
    try {
      await dbRunAsync('UPDATE pending_deposits SET data = ?, updated_at = CURRENT_TIMESTAMP WHERE unique_code = ?', [JSON.stringify(createResp || {}), ref_kode]);
    } catch (e) {
      console.warn('Failed update pending_deposits data', e && e.message);
    }
    ...
    try {
      ...
      if (createResp && createResp.data) {
        ...
        const url = d.payment_url || d.url || (d.data && d.data.url) || (d.invoice && d.invoice.url);
        if (url) {
          userMessage += `Silakan bayar melalui tautan berikut:\n${url}`;
          await ctx.reply(userMessage);
        } else if (d.qr) {
          userMessage += 'QRIS tersedia. Jika bot mendukung pengiriman image base64, tambahkan fitur ini.';
          await ctx.reply(userMessage);
        } else {
          userMessage += `Data payment gateway: ${JSON.stringify(d).slice(0,400)}...`;
          await ctx.reply(userMessage);
        }
      } else {
      ...
      console.warn('Failed send payment info to user', e && e.message);
      await ctx.reply('Transaksi dibuat, namun gagal mengirim detail pembayaran. Mohon cek status atau hubungi admin.');
    }

    delete violetDepositState[uid];
    return;
  } catch (err) {
    console.error('violet text handler error', err && err.message || err);
    return;
  }
});

// Callback endpoint for Violet Media
try {
  // ensure app exists
  if (typeof app !== 'undefined' && app) {
    app.post('/callback.php', async (req, res) => {
      try {
        const data = req.body;
        if (!data || !data.ref) {
          console.warn('Violet callback missing ref or body empty');
          return res.status(400).send('bad request');
        }
        const refid = String(data.ref);
        const incomingSignature = String(data.signature || '');

        // expected signature per docs: HMAC-SHA256(ref, apikey)
        const apikey = (typeof VIOLET_APIKEY !== 'undefined') ? VIOLET_APIKEY : (vars && vars.VIOLET_APIKEY);
        const expectedSig = require('crypto').createHmac('sha256', String(apikey)).update(refid).digest('hex');

        const remoteIp = (req.headers['x-forwarded-for'] || req.connection.remoteAddress || '').split(',')[0].trim();
        // optional trusted IPs check (default from vars or common ip)
        const trusted = (vars && vars.VIOLET_TRUSTED_IPS ? String(vars.VIOLET_TRUSTED_IPS) : '36.50.77.77').split(',').map(s=>s.trim()).filter(Boolean);

        if (incomingSignature !== expectedSig) {
          console.warn(`Violet callback signature mismatch for ref ${refid}. incoming=${incomingSignature} expected=${expectedSig} from ${remoteIp}`);
          return res.status(403).send('forbidden');
        }

        if (trusted.length > 0 && !trusted.includes(remoteIp)) {
          console.warn(`Violet callback from untrusted IP ${remoteIp} for ref ${refid}`);
          return res.status(403).send('forbidden');
        }
        ...
        let pending = null;
        try {
          pending = await dbGetAsync('SELECT * FROM pending_deposits WHERE unique_code = ?', [refid]);
        } catch (e) {
          console.warn('DB lookup pending_deposits failed', e && e.message);
        }

        if (!pending) {
          console.warn(`Violet callback for unknown ref ${refid}. Data:`, data);
        ...
        if (status === 'success') {
          ...
          } catch (e) { console.warn('Failed update pending_deposits status', e && e.message); }
          ...
          try {
            ...
            if (userRow) {
              ...
              } catch (e) { console.warn('Failed notify user after violet callback', e && e.message); }

              // notify admin group
              try {
                const gid = (vars && vars.GROUP_ID) ? Number(vars.GROUP_ID) : (typeof GROUP_ID !== 'undefined' ? GROUP_ID : null);
                if (gid) {
                  await bot.telegram.sendMessage(gid, `💳 TOPUP BERHASIL\\nUser: ${pending.user_id}\\nRef: ${refid}\\nJumlah: Rp${Number(pending.amount).toLocaleString('id-ID')}`);
                }
              } catch (e) { console.warn('Failed notify admin group after violet callback', e && e.message); }
            } else {
              console.error('User not found when crediting after violet callback', pending.user_id);
            }
          } catch (e) {
            console.error('Error crediting user after violet callback', e && e.message);
          }

          return res.status(200).send('OK');
        } else if (status === 'kadaluarsa' || status === 'expired') {
          try { await dbRunAsync('UPDATE pending_deposits SET status = ?, data = ?, updated_at = CURRENT_TIMESTAMP WHERE unique_code = ?', ['expired', JSON.stringify(data), refid]); } catch(e){}
          return res.status(200).send('OK');
        } else {
          try { await dbRunAsync('UPDATE pending_deposits SET status = ?, data = ?, updated_at = CURRENT_TIMESTAMP WHERE unique_code = ?', ['failed', JSON.stringify(data), refid]); } catch(e){}
          return res.status(200).send('OK');
        }

      } catch (err) {
        console.error('Violet callback handler error', err && err.message || err);
        return res.status(500).send('error');
      }
    });
  } else {
    console.warn('Could not register /callback.php because Express "app" was not found in runtime scope.');
  }
} catch (err) {
  console.warn('Failed to attach Violet callback endpoint:', err && err.message);
}

// Periodic cleanup for stuck sessions
setInterval(() => { try { /* clear short-lived session flags if any */ } catch(e){} }, 5*60*1000);