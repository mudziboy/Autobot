...
const {
  ...
  MERCHANT_ID,
  API_KEY,
  VIOLET_MEDIA_API_KEY,
  VIOLET_MEDIA_SECRET_KEY,
  VIOLET_MEDIA_CALLBACK_URL,
} = vars;

// 🔑 Violet Media keys are loaded from .vars.json as VIOLET_MEDIA_API_KEY, VIOLET_MEDIA_SECRET_KEY, VIOLET_MEDIA_CALLBACK_URL
// VIOLET_CONFIG_INSERTED
...
// const QRISPayment = require('qris-payment'); // removed by merge for Violet Media
...
// =========================================
// 🔔 Callback Payment Gateway (Violet Media)
// =========================================
app.post('/callback', async (req, res) => {
  const data = req.body;
  try {
    const signature = crypto
      .createHmac('sha256', VIOLET_MEDIA_API_KEY)
      .update(data.ref)
      .digest('hex');

    // Violet Media documented source IP: 36.50.77.77
    const remoteIp = req.headers['x-forwarded-for'] ? req.headers['x-forwarded-for'].split(',')[0].trim() : req.ip;

    if (data.signature !== signature || remoteIp !== '36.50.77.77') {
      logger.warn('Invalid callback signature or IP', { signature: data.signature, remoteIp });
      return res.status(403).json({ error: 'Invalid signature or IP' });
    }

    if (data.status === 'success') {
      // amount_merchant is the amount that goes to merchant; use that as saldo increment
      await dbRunAsync(`UPDATE users SET saldo = saldo + ? WHERE user_id = ?`, [data.amount_merchant, data.phone]);
      await dbRunAsync(`UPDATE pending_deposits SET status = 'PAID' WHERE unique_code = ?`, [data.ref]);
      await dbRunAsync(`INSERT INTO topup_log (user_id, username, amount, reference, created_at) VALUES (?, ?, ?, ?, datetime('now'))`, [data.phone, data.nama, data.amount_merchant, data.ref]);
      logger.info('Topup berhasil via callback', { user: data.phone, amount: data.amount_merchant, ref: data.ref });
    } else if (data.status === 'kadaluarsa') {
      await dbRunAsync(`UPDATE pending_deposits SET status = 'EXPIRED' WHERE unique_code = ?`, [data.ref]);
      logger.info('Topup kadaluarsa', { ref: data.ref });
    } else {
      logger.info('Callback diterima dengan status lain', { status: data.status, ref: data.ref });
    }

    return res.json({ success: true });
  } catch (err) {
    logger.error('Error handling callback:', err.message);
...
// =========================================
// 🔁 Fungsi Violet Media Payment (create transaction)
// =========================================
async function createVioletTransaction(user, amount) {
  const refKode = Date.now().toString();
  const signature = crypto
    .createHmac("sha256", VIOLET_MEDIA_SECRET_KEY)
    .update(refKode + VIOLET_MEDIA_API_KEY + amount)
    .digest("hex");

  const payload = {
    apikey: VIOLET_MEDIA_API_KEY,
    amount: amount,
    ref_kode: refKode,
    produk: "TopUp Saldo",
    nama: user.username || user.first_name || "User",
    phone: user.user_id,
    callback_url: VIOLET_MEDIA_CALLBACK_URL,
    signature
  };

  const res = await axios.post("https://violetmediapay.com/api/live/create", payload, {
    headers: { "Content-Type": "application/json" }
  });

  if (!res.data || res.data.status !== true) throw new Error("Gagal membuat transaksi Violet Media");
...
// =========================================
// 💳 Handler Custom TopUp (mengganti QRIS statis)
// =========================================
bot.action('topup_saldo', async (ctx) => {
  try {
    await ctx.reply('💰 Silakan masukkan nominal top up yang diinginkan (minimum Rp10.000).');
    ctx.session.awaitingTopup = true;
  } catch (err) {
    logger.error('Gagal menghandle topup_saldo:', err.message);
  }
});

bot.on('text', async (ctx) => {
  try {
    if (!ctx.session || !ctx.session.awaitingTopup) return;
    ctx.session.awaitingTopup = false;

    const raw = String(ctx.message.text || '').replace(/[^0-9]/g, '');
    const nominal = parseInt(raw, 10);
    if (isNaN(nominal) || nominal < 10000) {
      return ctx.reply('❌ Nominal tidak valid. Minimal Rp10.000.');
    }
    ...
    // create transaction
    const trx = await createVioletTransaction(user, nominal);

    // store pending deposit
    await dbRunAsync(`INSERT INTO pending_deposits (unique_code, user_id, amount, original_amount, timestamp, status) VALUES (?, ?, ?, ?, strftime('%s','now'), ?)`,
      [trx.ref_kode, userId, trx.nominal, nominal, 'PENDING']);

    const msg = `🧾 *TopUp Saldo*\n\nNominal: Rp${trx.nominal.toLocaleString('id-ID')}\n\nSilakan buka link berikut untuk menyelesaikan pembayaran:\n${trx.checkout_url}\n\nInvoice kadaluarsa: ${trx.expired_time}`;
    await ctx.reply(msg, { parse_mode: 'Markdown' });
  } catch (err) {
    logger.error('Gagal proses topup custom:', err.message);
    ctx.reply('❌ Gagal membuat transaksi, silakan coba lagi nanti.');
  }
});
...
db.all('SELECT * FROM pending_deposits WHERE status = "pending"', [], (err, rows) => {
  ...
  logger.info('Pending deposit loaded:', Object.keys(global.pendingDeposits).length);
   ...
   // apiKey: API_KEY,
   // baseQrString: /*DATA_QRIS_REMOVED*/,
 //   logoPath: 'logo.png'
//    });
