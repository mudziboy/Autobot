  ...
  try {
    ...
    try {
      ...
      await ctx.editMessageMedia({
        type: 'photo',
        media: qrisImageUrl,
        caption: '💰 *Silakan scan QRIS berikut untuk melakukan top-up saldo Anda.*\n\n📌 Setelah membayar, kirim bukti transfer untuk verifikasi dengan menyertakan username Telegram anda | Admin : @rahmarie. *Saldo akan ditambahkan ketika Admin online',
        parse_mode: 'Markdown'
      });
    } catch (imageError) {
...
bot.action(/reduce_saldo_(\d+)/, async (ctx) => {
  ...
  // Simpan state sementara agar input berikutnya diketahui konteksnya
  userState[ctx.chat.id] = { step: 'reduce_saldo', userId: userId };

  await ctx.reply('📉 *Silakan masukkan jumlah saldo yang ingin dikurangi:*', {
...
// === PROSES INPUT SALDO YANG DIKURANGI ===
bot.on('text', async (ctx) => {
  const state = userState[ctx.chat.id];
  if (!state || state.step !== 'reduce_saldo') return; // kalau bukan mode pengurangan, lanjut ke handler lain
  ...
  if (isNaN(amount) || amount <= 0) {
    await ctx.reply('⚠️ Masukkan jumlah saldo yang valid (angka positif).');
    return;
  }

  try {
    // Ambil saldo user saat ini
    const user = await db.get('SELECT saldo FROM users WHERE user_id = ?', [userId]);
    if (!user) {
      await ctx.reply('❌ Pengguna tidak ditemukan di database.');
      delete userState[ctx.chat.id];
      return;
    }
    ...
    if (user.saldo < amount) {
      await ctx.reply(`🚫 Saldo user tidak mencukupi. Saldo saat ini: Rp${user.saldo.toLocaleString('id-ID')}`);
      delete userState[ctx.chat.id];
      return;
    }
    ...
    await db.run('UPDATE users SET saldo = ? WHERE user_id = ?', [newSaldo, userId]);

    logger.info(`Saldo user ${userId} dikurangi sebesar ${amount} oleh admin ${ctx.from.id}.`);
    await ctx.reply(
      `✅ Berhasil mengurangi saldo user ID *${userId}* sebesar *Rp${amount.toLocaleString('id-ID')}*.\n💰 Sisa saldo: *Rp${newSaldo.toLocaleString('id-ID')}*`,
      { parse_mode: 'Markdown' }
    );
  } catch (err) {
    logger.error(`Gagal mengurangi saldo user ${userId}: ${err.message}`);
    await ctx.reply('❌ Terjadi kesalahan saat mengurangi saldo.');
  } finally {
    delete userState[ctx.chat.id]; // Hapus state agar tidak bentrok di input berikutnya
  }
});
...
bot.action(/add_saldo_(\d+)/, async (ctx) => {
  ...
  // Simpan state untuk proses input saldo
...
// === PROSES INPUT TAMBAH SALDO ===
bot.on('text', async (ctx) => {
  const state = userState[ctx.chat.id];
  if (!state || (state.step !== 'add_saldo' && state.step !== 'reduce_saldo')) return; // hanya proses jika sedang mode saldo
  ...
  try {
    ...
    // Jika sedang mode tambah saldo
    if (state.step === 'add_saldo') {
      newSaldo = user.saldo + amount;
      actionType = 'menambahkan';
      messageAdmin = `✅ Berhasil *menambahkan* saldo user ID *${userId}* sebesar *Rp${amount.toLocaleString('id-ID')}*.\n💰 Saldo sekarang: *Rp${newSaldo.toLocaleString('id-ID')}*`;
      messageUser = `💰 *Saldo Anda telah ditambahkan!*\n\n📥 Penambahan: *Rp${amount.toLocaleString('id-ID')}*\n💳 Saldo saat ini: *Rp${newSaldo.toLocaleString('id-ID')}*`;

    // Jika sedang mode kurangi saldo
    } else if (state.step === 'reduce_saldo') {
      if (user.saldo < amount) {
        await ctx.reply(`🚫 Saldo user tidak mencukupi. Saldo saat ini: Rp${user.saldo.toLocaleString('id-ID')}`);
        delete userState[ctx.chat.id];
        return;
      ...
      newSaldo = user.saldo - amount;
      actionType = 'mengurangi';
      messageAdmin = `✅ Berhasil *mengurangi* saldo user ID *${userId}* sebesar *Rp${amount.toLocaleString('id-ID')}*.\n💰 Sisa saldo: *Rp${newSaldo.toLocaleString('id-ID')}*`;
      messageUser = `⚠️ *Saldo Anda telah dikurangi oleh admin!*\n\n📤 Pengurangan: *Rp${amount.toLocaleString('id-ID')}*\n💳 Sisa saldo: *Rp${newSaldo.toLocaleString('id-ID')}*`;
    }

    // Update saldo di database
    await db.run('UPDATE users SET saldo = ? WHERE user_id = ?', [newSaldo, userId]);
    logger.info(`Admin ${ctx.from.id} ${actionType} saldo user ${userId} sebesar ${amount}`);

    // Notifikasi ke admin
    await ctx.reply(messageAdmin, { parse_mode: 'Markdown' });

    // Kirim notifikasi otomatis ke user target
    await bot.telegram.sendMessage(userId, messageUser, { parse_mode: 'Markdown' })
      .catch(err => logger.warn(`Tidak bisa kirim notifikasi ke user ${userId}: ${err.message}`));

  } catch (err) {
    logger.error(`Gagal memproses saldo user ${userId}: ${err.message}`);
    await ctx.reply('❌ Terjadi kesalahan saat memproses saldo.');
  } finally {
    delete userState[ctx.chat.id]; // hapus state agar tidak bentrok
  }
});
