...
app.post("/callback.php", async (req, res) => {
  const data = req.body;
  const clientIp =
    req.headers["cf-connecting-ip"] ||  // 🌐 IP asli dari Cloudflare
    req.headers["x-forwarded-for"]?.split(",")[0]?.trim() ||
    req.socket.remoteAddress;

  const API_KEY = VIOLET_API_KEY.trim();
  const ALLOWED_IPS = ["202.155.132.37", "2001:df7:5300:9::122"];
  ...
  try {
    log(`📩 [CALLBACK] Data diterima dari ${clientIp}: ${JSON.stringify(data)}`);

    const ref = String(data.ref || data.ref_kode || "");
    const signature = String(data.signature || "");
    const status = (data.status || "").toLowerCase();

    // 🔑 Buat signature lokal berdasarkan dokumen resmi VioletMedia (gunakan secret key + raw body)
const localSignature = crypto
  ...
  .digest("hex");
    ...
    log(`→ Remote Signature: ${signature}`);
    log(`→ IP Valid: ${isIpAllowed}`);

    if (status === "success" && isSignatureValid && isIpAllowed) {
      log(`✅ Callback sukses — transaksi ${ref} diverifikasi.`);
      // Tambahkan logika update saldo user berdasarkan ref
      // Contoh: await dbRunAsync("UPDATE users SET saldo = saldo + ? WHERE user_id = ?", [data.amount_received, userId]);
      return res.json({ status: true });
    } else if (status === "kadaluarsa" && isSignatureValid && isIpAllowed) {
...
🏪 𝗣𝗿𝗼𝘁𝗼𝗰𝗼𝗹: SSH
⏳ 𝗗𝘂𝗿𝗮𝘀𝗶: 60 Menit
🕒 𝗪𝗮𝗸𝘁??: ${new Date().toLocaleString('id-ID')}
━━━━━━━━━━━━━━━━━━━━━━━
          `.trim();
