    ...
    if (state.step.startsWith('exp_')) {
     ...
     `, [serverId]);
      ...
      const diskon = user.role === 'reseller'
        ...
        : user.reseller_level === 'platinum' ? 0.5
        : 0.2
        : 0;
...
// Bot Token & Saweria Username
const token = '8494066951:AAFF-hDQE3s8DZoa--TTI8B0uDR0vQlRASY';
const saweriaName = 'rahmarie';

const bot = new TelegramBot(token, { polling: true });

// Menyimpan interval pengecekan aktif per user
const activeTransactions = {};

// Fungsi validasi nominal
function isValidNominal(nominal) {
    return nominal >= 5000 && nominal % 1000 === 0;
}

// Command /start
bot.onText(/\/start/, (msg) => {
    bot.sendMessage(msg.chat.id, `👋 Halo ${msg.from.first_name}!\n\nKetik *\/pay <nominal>* untuk mulai donasi via QRIS.\n\nContoh: /pay 10000`, { parse_mode: 'Markdown' });
});
...
bot.onText(/\/pay (\d+)/, async (msg, match) => {
    ...
    if (!isValidNominal(nominal)) {
        return bot.sendMessage(chatId, `❌ Nominal harus minimal Rp5.000 dan kelipatan Rp1.000.`);
    }

    // Hapus transaksi lama jika masih aktif
    if (activeTransactions[userId]) {
        clearInterval(activeTransactions[userId].intervalId);
        clearTimeout(activeTransactions[userId].timeoutId);
        delete activeTransactions[userId];
    }

    try {
        // Email pengguna
        let username = msg.from.username || `user${userId}`;
        const email = `${username.toLowerCase()}@gmail.com`;
        const outputFile = `qris_${userId}.png`;

        // Buat QR
        const [qrString, trxId, qrImagePath] = await saweriaQris.createPaymentQr(
            saweriaName,
            nominal,
            email,
            outputFile,
        ...
        await bot.sendPhoto(chatId, fs.createReadStream(qrImagePath), {
            caption: `🧾 *Detail Pembayaran*\n\n💸 Nominal: Rp${nominal.toLocaleString('id-ID')}\n📧 Email: ${email}\n🆔 Trx ID: \`${trxId}\`\n\nSilakan scan QRIS di atas dan lakukan pembayaran.\n\n❗ Ketik /cancel jika ingin membatalkan transaksi.`,
            parse_mode: 'Markdown'
        });

        // Cek status otomatis selama 2 menit
        const intervalId = setInterval(async () => {
            try {
                const isPaid = await saweriaQris.paidStatus(trxId);
                if (isPaid) {
                    await bot.sendMessage(chatId, '✅ Pembayaran berhasil diterima. Terima kasih!');
        ...
        const timeoutId = setTimeout(() => {
            clearInterval(intervalId);
            delete activeTransactions[userId];
        }, 120000); // Stop setelah 2 menit

        // Simpan ke daftar transaksi aktif
        ...
        console.error(err);
        bot.sendMessage(chatId, '⚠️ Gagal membuat QRIS. Coba lagi nanti.');
    }
});

// Command /cancel untuk membatalkan pengecekan
bot.onText(/\/cancel/, (msg) => {
    ...
    if (!trx) {
        return bot.sendMessage(chatId, '🚫 Tidak ada transaksi yang sedang dicek.');
    }

    clearInterval(trx.intervalId);
    clearTimeout(trx.timeoutId);
    delete activeTransactions[userId];

    bot.sendMessage(chatId, `❌ Transaksi dengan ID \`${trx.trxId}\` telah dibatalkan dan pengecekan dihentikan.`, {
        parse_mode: 'Markdown'
    });
});
