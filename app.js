...
bot.action('topup_saldo', async (ctx) => {
  try {
    ...
    const message = await ctx.editMessageMedia({
      type: 'photo',
      media: qrisImageUrl,
      caption: '💰 *Silakan scan QRIS berikut untuk melakukan top-up saldo Anda.*\n\n📌 Setelah membayar, kirim bukti dengan mengirimkan pesan foto transfer ke bot ini.\nAdmin online 24/7 kecuali tidur',
      parse_mode: 'Markdown'
    });
...
bot.on('callback_query', async (ctx) => {
  ...
  if (data.startsWith('verif_')) {
    ...
    // Mulai keyboard nomor khusus top-up user
...
// ===== FUNCTION UTAMA TOP-UP =====
async function handleTopup(userId, amount, adminCtx) {
  if (!global.depositState[userId]) return;

  // Tambahkan saldo user di DB / JSON
