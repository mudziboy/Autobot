  ...
  try {
    ...
    global.depositState[userId].timeout = setTimeout(async () => {
      if (global.depositState[userId]) {
        ...
        logger.info(`⏱️ User ${userId} top-up expired.`);
      }
    }, 5 * 60 * 1000);
...
bot.on(['photo', 'document'], async (ctx) => {
  ...
  const adminId = YOUR_ADMIN_TELEGRAM_ID; // ganti sesuai admin
  ...
  // Kirim tombol inline untuk verifikasi / tolak
  await ctx.telegram.sendMessage(adminId,
    `📥 User @${ctx.from.username || userId} mengirim bukti top-up.\n\nUserID: ${userId}`,
    {
      reply_markup: {
        inline_keyboard: [
          [
            { text: '✅ Verifikasi', callback_data: `verifask_${userId}` },
  ...
  global.depositState[userId].ctx = ctx; // simpan ctx untuk notif nanti
});
...
bot.on('callback_query', async (ctx) => {
  ...
  // Admin klik verifikasi → minta input nominal
  if (data.startsWith('verifask_')) {
    const userId = data.split('_')[1];
    if (!global.depositState[userId] || global.depositState[userId].action !== 'await_admin_verif') {
      await ctx.answerCbQuery('❌ Tidak ada top-up menunggu verifikasi.');
      return;
    }
    ...
    global.depositState[userId].action = 'await_admin_input_nominal';
    global.depositState[userId].adminId = adminId;

  } else if (data.startsWith('tolak_')) {
    const userId = data.split('_')[1];
    if (!global.depositState[userId] || !global.depositState[userId].action.startsWith('await_admin')) {
      await ctx.answerCbQuery('❌ Tidak ada top-up menunggu verifikasi.');
      return;
    }

    const userCtx = global.depositState[userId].ctx;
    await userCtx.reply('❌ Top-up Anda *ditolak* oleh admin. Silakan cek kembali bukti transfer Anda.', { parse_mode: 'Markdown' });
...
bot.on('text', async (ctx) => {
  const adminId = ctx.from.id;
  // Cari user yang menunggu input nominal dari admin ini
  const pendingUser = Object.keys(global.depositState).find(uid => 
    global.depositState[uid].action === 'await_admin_input_nominal' &&
    global.depositState[uid].adminId === adminId
  );
  if (!pendingUser) return;

  const amount = parseFloat(ctx.message.text.replace(/[^\d]/g, ''));
  if (isNaN(amount) || amount <= 0) return ctx.reply('❌ Nominal tidak valid, masukkan angka lebih dari 0.');

  // Tambah saldo user di DB
  // db.addBalance(pendingUser, amount);

  const userCtx = global.depositState[pendingUser].ctx;
  await userCtx.reply(`✅ Top-up saldo Anda sebesar Rp${amount.toLocaleString()} telah diverifikasi oleh admin.`);

  // Hapus QRIS user
  await userCtx.deleteMessage(global.depositState[pendingUser].messageId).catch(() => {});

  delete global.depositState[pendingUser];

  await ctx.reply(`✅ Top-up User ${pendingUser} sebesar Rp${amount.toLocaleString()} berhasil diverifikasi.`);
});
