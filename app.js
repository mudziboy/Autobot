...
// ===== DATABASE SQLITE =====
const dataDir = path.resolve('/root/BotVPN2/data'); 
if (!fs.existsSync(dataDir)) fs.mkdirSync(dataDir, { recursive: true });

const dbPath = path.join(dataDir, 'sellvpn.db');

// Gunakan db hanya jika belum ada
...
bot.on('callback_query', async (ctx) => {
  ...
  // === TOMBOL NOMOR ===
    ...
    if (key === 'ok') {
      ...
      await ctx.editMessageText(`💰 Nominal yang diketik: Rp${global.depositState[userId].inputNominal}`, { reply_markup: { inline_keyboard: keyboard_nomor_topup(userId) } });
      await ctx.answerCbQuery();
    }
  }
});
...
async function handleTopup(userId, amount, adminCtx) {
  ...
  try {
    // Ambil user dulu
    db.get('SELECT saldo FROM users WHERE userId = ?', [userId], async (err, row) => {
      if (err) {
        console.error(`❌ Gagal baca user ${userId}:`, err);
        if (userCtx) await userCtx.reply('❌ Terjadi kesalahan saat membaca saldo.');
        return;
      }

      if (row) {
        // User sudah ada, update saldo
        const newSaldo = (row.saldo || 0) + amount;
        db.run('UPDATE users SET saldo = ?, username = ? WHERE userId = ?', [newSaldo, userCtx?.from?.username || '', userId], async (err) => {
          if (err) {
            console.error(`❌ Gagal update saldo user ${userId}:`, err);
            if (userCtx) await userCtx.reply('❌ Terjadi kesalahan saat menambahkan saldo.');
            return;
          }

          if (userCtx) {
            await userCtx.reply(`✅ Top-up Rp${amount.toLocaleString()} berhasil diverifikasi!\n💳 Saldo sekarang: Rp${newSaldo.toLocaleString()}`);
            await userCtx.deleteMessage(global.depositState[userId].messageId).catch(() => {});
          }
        ...
        db.run('INSERT INTO users (userId, username, saldo, role, createdAt) VALUES (?, ?, ?, ?, ?)', 
          [userId, userCtx?.from?.username || '', amount, 'User', new Date().toISOString()], async (err) => {
          if (err) {
          ...
          if (userCtx) {
            await userCtx.reply(`✅ Top-up Rp${amount.toLocaleString()} berhasil diverifikasi!\n💳 Saldo sekarang: Rp${amount.toLocaleString()}`);
            await userCtx.deleteMessage(global.depositState[userId].messageId).catch(() => {});
          }
