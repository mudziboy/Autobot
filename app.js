...
async function requestVioletTransaction(userId, amount, email, phone, paymentMethodCode) {
    ...
    // --- SIMPAN TRANSAKSI KE DATABASE (PENDING) ---
    try {
        const timestamp = Date.now();
        await dbRunAsync(`
            INSERT INTO pending_deposits (unique_code, user_id, amount, original_amount, timestamp, status, qr_message_id)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        `, [ref_kode, userId, amount, amount, timestamp, 'pending', null]);

        logger.info(`💾 Transaksi ${ref_kode} disimpan ke pending_deposits (status pending).`);
    } catch (dbErr) {
        logger.error(`❌ Gagal menyimpan transaksi ${ref_kode} ke pending_deposits: ${dbErr.message}`);
    }
