...
app.post('/callback.php', async (req, res) => {
  try {
    ...
    if (receivedSignature !== calculatedSignature) {
      logger.warn('❌ Signature tidak valid dari callback Violet.');
    ...
    if (data.status?.toLowerCase() === 'success' || data.status_code === 200) {
      ...
      const userId = ref.split('_')[1]; // karena format: sagivpn_userId_timestamp
      ...
      await safeSend(bot, userId, `✅ *Top-Up Berhasil!*\n💰 Rp${amount.toLocaleString('id-ID')}\nRef: ${ref}`, { parse_mode: 'Markdown' });
