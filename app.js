...
bot.action(/select_channel_(\w+)/, async (ctx) => {
    ...
    try {
        ...
        // 🎯 LOGIKA PERBAIKAN: Gunakan kode pembayaran yang dikonfirmasi (QRIS)
        let paymentCodeToSend = channelName; 

        if (channelName === 'QRIS') {
            // Gunakan kode QRIS yang dikonfirmasi benar dari dokumentasi
        ...
        // Panggil API dengan kode yang telah kita putuskan
        ...
        if (response.status === true) {
            ...
            // Gunakan data.expired yang sudah dalam detik (sesuai API)
...
`.trim();
            ...
            // PERBAIKAN AKSES LINK QRIS: Gunakan data.target untuk gambar
            if (data.target) { // <--- Cek data.target, bukan data.qr_string
            ...
            const checkoutLink = data.checkout_url || data.target; // Gunakan checkout_url untuk tombol link
