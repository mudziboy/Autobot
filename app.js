...
// Gunakan db global, deklarasikan sekali
if (!global.db) {
...
async function handleTopup(userId, amount, adminCtx) {
  const userCtx = global.depositState[userId]?.ctx;
  try {
    global.db.get('SELECT saldo FROM users WHERE userId = ?', [userId], async (err, row) => {
      if (err) {
      ...
      if (row) {
        ...
        global.db.run('UPDATE users SET saldo = ?, username = ? WHERE userId = ?', [newSaldo, userCtx?.from?.username || '', userId], async (err) => {
          if (err) {
        ...
        global.db.run('INSERT INTO users (userId, username, saldo, role, createdAt) VALUES (?, ?, ?, ?, ?)', 
