...
// ===== VIOLETMEDIA UTILITY (Lanjutan) =====
// ... (Pastikan createVioletSignature TIDAK diubah, dan tetap menggunakan VIOLET_SECRET_KEY.trim())
...
async function requestVioletTransaction(userId, amount, email, phone, paymentMethodCode) {
    ...
    // ❌ HILANGKAN SECRET_KEY DARI BODY REQUEST INI. SECRET_KEY HANYA UNTUK HITUNG SIGNATURE.
    // formData.append("secret_key", VIOLET_SECRET_KEY.trim()); <--- BARIS INI DIHAPUS
    ...
    try {
        ...
        if (response.data.status !== true) {
             // 💡 Log payload yang dikirim (untuk debug lanjutan)
             logger.error(`❌ VIOLET Transaksi Gagal. Pesan: ${response.data.message || JSON.stringify(response.data)}, Payload: ${formData.toString()}`);
        }
