...
// GANTI BLOK INI di dalam bot.action(/select_channel_(\w+)/)

        // 🎯 LOKASI PERBAIKAN: Tentukan kode pembayaran yang akan dikirim ke API
        let paymentCodeToSend = channelName; // Default: kirim 'QRIS'
        
        // KODE BARU: Jika nama yang dipilih adalah 'QRIS', kirimkan 'QRIS'
        if (channelName === 'QRIS') {
             // Kirim kode 'QRIS' yang sudah dikonfirmasi benar dari dokumentasi
        ...
        // Panggil API dengan kode 'QRIS'
        const response = await requestVioletTransaction(
            userId, 
            amount, 
            contact.email, 
            contact.phone, 
            paymentCodeToSend // Ini adalah 'QRIS'
        );
