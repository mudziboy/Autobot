...
bot.action(/select_channel_(\w+)/, async (ctx) => {
    ...
    try {
        ...
        if (response.status === true) {
            ...
            // 🎯 PERBAIKAN TYPO: Date.now() di Baris 5322
            await dbRunAsync(
                `INSERT INTO pending_deposits (unique_code, user_id, amount, original_amount, timestamp, status, qr_message_id)
                 VALUES (?, ?, ?, ?, ?, ?, ?)`,
                [refId, userId, totalBayar, amount, Date.now(), 'pending_auto', 0] // <-- DIBETULKAN DARI DateAt.now()
            );
...
`.trim();
        ...
        logger.error('❌ Gagal proses transaksi otomatis:', e.message); // Sekarang e.message akan lebih jelas
