...
const dbPath = path.resolve('/root/BotVPN2/sellvpn.db');
const db = new sqlite3.Database(dbPath, (err) => {
  if (err) console.error('❌ Kesalahan koneksi SQLite3:', err.message);
  else console.log('✅ Terhubung ke SQLite3');
});
...
db.run(`
  CREATE TABLE IF NOT EXISTS users (
    ...
    saldo INTEGER DEFAULT 0,
    totalAkunDibuat INTEGER DEFAULT 0,
    role TEXT DEFAULT 'User',
...
console.log(`[DEBUG] Global depositState siap digunakan.`);
...
bot.action('topup_saldo', async (ctx) => {
  try {
    await ctx.answerCbQuery();
    const userId = ctx.from.id;
    console.log(`[DEBUG] User ${userId} menekan tombol topup_saldo`);

    global.depositState[userId] = { action: 'await_bukti', timeout: null };
    ...
    const message = await ctx.editMessageMedia({
      type: 'photo',
      media: qrisImageUrl,
      caption: '💰 *Silakan scan QRIS berikut untuk melakukan top-up saldo Anda.*\n\n📌 Setelah membayar, kirim bukti dengan membalas pesan ini dan wajib sertakan foto transfer.',
      parse_mode: 'Markdown'
    });

    global.depositState[userId].messageId = message.message_id;
    console.log(`[DEBUG] Pesan QRIS dikirim untuk user ${userId}, message_id=${message.message_id}`);

    // Timeout 5 menit
    global.depositState[userId].timeout = setTimeout(async () => {
      if (global.depositState[userId]) {
        console.log(`[DEBUG] Top-up user ${userId} expired (5 menit)`);
        await ctx.deleteMessage(global.depositState[userId].messageId).catch(() => {});
        delete global.depositState[userId];
        await ctx.reply('⌛ Top-up Anda *expired* karena tidak mengirim bukti transfer dalam 5 menit.', { parse_mode: 'Markdown' });
      }
    }, 5 * 60 * 1000);

  } catch (error) {
    console.error('❌ Kesalahan saat memulai top-up:', error);
    await ctx.editMessageText('❌ *Gagal memproses top-up. Silakan coba lagi nanti.*', { parse_mode: 'Markdown' });
  }
});
...
bot.on(['photo', 'document'], async (ctx) => {
  ...
  console.log(`[DEBUG] User ${userId} mengirim bukti transfer`);

  clearTimeout(global.depositState[userId].timeout);

  const adminId = 8118068872; // ganti sesuai admin ID
  await ctx.reply('✅ Bukti transfer diterima, diteruskan ke admin untuk verifikasi.');
  await ctx.forwardMessage(adminId, ctx.chat.id, ctx.message.message_id);
  ...
  console.log(`[DEBUG] Menunggu verifikasi admin untuk user ${userId}`);
});

// ===== CALLBACK ADMIN =====
bot.on('callback_query', async (ctx) => {
  const data = ctx.callbackQuery.data;
  console.log(`[DEBUG] callback_query diterima: ${data}`);

  // === ADMIN VERIFIKASI ===
  if (data.startsWith('verif_')) {
    const userId = data.split('_')[1];
    console.log(`[DEBUG] Admin menekan verif untuk user ${userId}`);

    if (!global.depositState[userId] || global.depositState[userId].action !== 'await_admin_verif') {
      console.log(`[DEBUG] Tidak ada depositState aktif untuk user ${userId}`);
      return;
    }
  ...
  // === ADMIN TOLAK ===
    ...
    const userId = data.split('_')[1];
    console.log(`[DEBUG] Admin menolak topup user ${userId}`);

    if (!global.depositState[userId]) return;
    const userCtx = global.depositState[userId].ctx;
    await userCtx.reply('❌ Top-up Anda *ditolak* oleh admin.', { parse_mode: 'Markdown' });
    await userCtx.deleteMessage(global.depositState[userId].messageId).catch(() => {});
    delete global.depositState[userId];
  ...
  // === TOMBOL NOMOR ===
    ...
    if (key === 'ok') {
      ...
      console.log(`[DEBUG] Admin tekan OK untuk user ${userId}, nominal=${amount}`);
      ...
      global.depositState[userId].inputNominal = '';
      await ctx.editMessageText(`💰 Nominal yang diketik: Rp0`, {
        reply_markup: { inline_keyboard: keyboard_nomor_topup(userId) }
      });
      await ctx.answerCbQuery();
      ...
      await ctx.editMessageText(`💰 Nominal yang diketik: Rp${global.depositState[userId].inputNominal}`, {
        reply_markup: { inline_keyboard: keyboard_nomor_topup(userId) }
      });
      await ctx.answerCbQuery();
    }
...
// ===== HANDLE TOP-UP DAN UPDATE SALDO (SQLite) =====
async function handleTopup(userId, amount, adminCtx) {
  console.log(`[DEBUG] handleTopup() dijalankan untuk user ${userId}, amount=${amount}`);

  try {
    // Pastikan user ada
    db.get('SELECT * FROM users WHERE userId = ?', [userId], (err, row) => {
      if (err) return console.error(`❌ Gagal baca user ${userId}:`, err.message);

      if (!row) {
        db.run(
          'INSERT INTO users(userId, saldo, totalAkunDibuat, role, username, createdAt) VALUES (?, ?, ?, ?, ?, ?)',
          [userId, 0, 0, 'User', '', new Date().toISOString()],
          function (err2) {
            if (err2) console.error(`❌ Gagal buat user ${userId}:`, err2.message);
            else console.log(`[DEBUG] User ${userId} dibuat di SQLite`);
          }
        );
      }

      // Update saldo
      db.run('UPDATE users SET saldo = saldo + ? WHERE userId = ?', [amount, userId], function (err3) {
        if (err3) return console.error(`❌ Gagal update saldo ${userId}:`, err3.message);

        console.log(`✅ Saldo user ${userId} bertambah Rp${amount.toLocaleString('id-ID')}`);

        const userCtx = global.depositState[userId]?.ctx;
        if (userCtx) {
          // Ambil saldo terbaru
          db.get('SELECT saldo FROM users WHERE userId = ?', [userId], async (err4, row2) => {
            const saldoTerbaru = row2?.saldo || 0;
            await userCtx.reply(`✅ Top-up Rp${amount.toLocaleString('id-ID')} berhasil diverifikasi!\n💳 Saldo sekarang: Rp${saldoTerbaru.toLocaleString('id-ID')}`);
            await userCtx.deleteMessage(global.depositState[userId].messageId).catch(() => {});
          });
        } else {
          console.log(`[DEBUG] ctx user ${userId} hilang, notifikasi tidak terkirim`);
        }

        delete global.depositState[userId];
        adminCtx.editMessageText(`✅ Top-up User ${userId} sebesar Rp${amount.toLocaleString('id-ID')} berhasil diverifikasi.`).catch(() => {});
      });
    });
  } catch (err) {
    console.error(`❌ handleTopup() error untuk user ${userId}:`, err);
  }
}
