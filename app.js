...
if (isSignatureValid && isIpAllowed) {
  if (status === "success") {
    ...
    // === START: CALLBACK LOGIC DENGAN ASYNC/AWAIT ===
    try {
      ...
      // 4️⃣ Kirim Notifikasi (Menggunakan fungsi Plain Text yang sudah ada)
      ...
      // Mengembalikan 200 agar VioletMedia tidak mengulang callback, karena sudah ada log
      return res.json({ status: true, message: 'Internal Server Error' }); 
    }
    // === END CALLBACK LOGIC DENGAN ASYNC/AWAIT ===
  } 
  
  // Logika status lain (kadaluarsa, dll)
  ...
  // Logika jika Signature/IP Invalid (tetap sama)
