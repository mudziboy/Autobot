...
// ==== CALLBACK VIOLET MEDIA ====
app.post('/callback.php', async (req, res) => {
  try {
    const rawBody = req.rawBody?.toString() || '';
    const receivedSignature = req.headers['x-signature'] || req.headers['x-signature-hmac-sha256'];

    // Buat signature lokal
    const calculatedSignature = crypto
      .createHmac('sha256', VIOLET_SECRET_KEY.trim())
      .update(rawBody)
      .digest('hex');

    // ⚙️ MODE DEBUG: jika tidak ada signature, tetap izinkan untuk pengujian manual
    if (receivedSignature && receivedSignature !== calculatedSignature) {
      logger.warn('❌ Signature callback Violet tidak valid');
      return res.status(403).send('Invalid signature');
    }

    const data = req.body;
    logger.info(`[CALLBACK] Data diterima: ${JSON.stringify(data)}`);

    // Proses jika status sukses
    if (data.status?.toLowerCase() === 'success' || data.status_code === 200) {
      const ref = data.ref_kode;
      const amount = parseInt(data.nominal);
      const userId = ref.split('_')[1]; // sagivpn_userId_timestamp

      await dbRunAsync(`UPDATE users SET saldo = saldo + ? WHERE user_id = ?`, [amount, userId]);
      await dbRunAsync(`INSERT INTO topup_log (user_id, amount, reference, created_at) VALUES (?, ?, ?, datetime('now'))`,
        [userId, amount, ref]);
      ...
      logger.info(`✅ Callback sukses — saldo user ${userId} bertambah Rp${amount}`);
    }

    res.send('OK');
  } catch (err) {
    logger.error('❌ Gagal proses callback Violet:', err.message);
