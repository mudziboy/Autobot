...
app.post("/callback", async (req, res) => {
  // ... (Kode Validasi Signature dan IP Tetap) ...
  
  if (isSignatureValid && isIpAllowed) {
      if (status === "success") {
          log(`✅ Callback sukses — transaksi ${ref} diverifikasi.`);
          
          // === START: CALLBACK LOGIC VIOLETMEDIA (REVISI) ===
          try {
              const GROUP_ID_ADMIN = (global.vars?.GROUP_ID || process.env.GROUP_ID || adminIds[0] || "").trim();

              // 1. Ambil transaksi dari pending_deposits menggunakan promisify
          ...
          // === END CALLBACK LOGIC VIOLETMEDIA (REVISI) ===

          return res.json({ status: true }); // Wajib mengembalikan 200/true
      } else if (status === "kadaluarsa" || status === "expired") {
          log(`⚠️ Callback kadaluarsa — transaksi ${ref} ditandai expired.`);
          
          // (Opsional: Update status di DB menjadi 'expired' jika diperlukan)
