...
async function handleTopup(userId, amount, adminCtx) {
  ...
  try {
    db.serialize(() => {
      // Pastikan user ada, gunakan UPSERT SQLite
      db.run(`
        INSERT INTO users (userId, username, saldo, role, createdAt)
        VALUES (?, ?, ?, ?, ?)
        ON CONFLICT(userId) DO UPDATE SET saldo = saldo + ?,
        username = excluded.username
      `, [userId, userCtx?.from?.username || '', amount, 'User', new Date().toISOString(), amount], function(err) {
        if (err) {
          console.error(`❌ Gagal update saldo user ${userId}:`, err);
          if (userCtx) userCtx.reply('❌ Terjadi kesalahan saat menambahkan saldo.');
          return;
        }
