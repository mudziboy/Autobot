...
bot.action(/select_channel_(\w+)/, async (ctx) => {
    ...
    try {
        ...
        if (response.status === true) {
            ...
            // Gunakan 'nominal' dari log terbaru, bukan 'nominal_akhir'
            const totalBayar = data.nominal; 
            // Gunakan 'expired_time' dari log terbaru
            const expiredAt = new Date(data.expired_time).toLocaleString('id-ID'); 
            ...
            global.depositState[userId] = {
                ...
                expired: Date.parse(data.expired_time) 
            };
            ...
            const petunjukText = data.petunjuk 
                ...
                : 'Silakan klik tombol Lanjut ke Pembayaran di bawah atau scan QRIS di atas (jika tersedia).';

            // Tampilkan Invoice
            // Gunakan escapeMarkdownV2 agar lebih aman dari karakter Markdown
            const invoiceText = `
🧾 *INVOICE TOP-UP OTOMATIS*
━━━━━━━━━━━━━━━━━━━━━━━
🌐 *Metode:* ${escapeMarkdownV2(data.code_payment)}
💵 *Total Bayar:* Rp ${escapeMarkdownV2(totalBayar.toLocaleString('id-ID'))}
🕒 *Batas Bayar:* ${escapeMarkdownV2(expiredAt)}
🔖 *Ref ID:* \`${escapeMarkdownV2(refId)}\`
━━━━━━━━━━━━━━━━━━━━━━━
*Instruksi Pembayaran:* ${escapeMarkdownV2(petunjukText)}
`.trim();
            
            const keyboard = [];
            
            // **PERBAIKAN KRITIS: KIRIM GAMBAR DULU DENGAN TRY/CATCH**
            if (data.target) {
                const qrLink = data.target; 
                try {
                    await ctx.replyWithPhoto(
                        { url: qrLink },
                        { caption: "Scan QRIS di atas" } // Biarkan caption sederhana
                    );
                    logger.info(`✅ Gambar QRIS berhasil dikirim ke user ${userId}`);
                } catch (e) {
                    logger.error(`❌ Gagal mengirim Gambar QRIS ke user ${userId}. Gunakan link sebagai fallback. Error: ${e.message}`);
                    await ctx.reply(`❌ Gambar QRIS gagal ditampilkan. Anda bisa menggunakan link checkout untuk melihat QRIS.`);
                }
            }

            // Tombol ke halaman pembayaran
            const checkoutLink = data.checkout_url || data.target; 
            keyboard.push([{ text: '🔗 Lanjut ke Pembayaran', url: checkoutLink }]);
            
            // Kirim pesan invoice utama (menggunakan MarkdownV2)
