...
💳 *Saldo sekarang:* Rp${newSaldo.toLocaleString('id-ID')}
      `.trim();
      await safeSend(bot, user_id, successMessage, { parse_mode: 'MarkdownV2' });
    }
    ...
    // 7. Kirim Notifikasi ke Grup (opsional, pastikan GROUP_ID ada)
    if (typeof GROUP_ID !== 'undefined' && GROUP_ID) {
      const adminMention = adminCtx.from.username ? `@${adminCtx.from.username}` : escapeMarkdownV2(adminCtx.from.first_name);
      const userMention = userCtx?.from.username ? `@${userCtx.from.username}` : escapeMarkdownV2(userCtx?.from.first_name || `ID ${user_id}`);
