...
// ===========================
// 📦 CALLBACK VIOLET MEDIA (Versi Final Aman)
// ===========================
app.post("/callback.php", async (req, res) => {
  ...
  try {
    ...
    // 🧾 Ambil signature yang dikirim Violet (prioritas: header)
    ...
    // 🔑 Buat signature lokal berdasarkan dokumen resmi (secret key + raw body asli)
    const localSignature = crypto
      .createHmac("sha256", SECRET_KEY)
      .update(req.rawBody)
      .digest("hex");
      
    log(`→ clientIp detected: ${clientIp}`);
    ...
    if (isSignatureValid && isIpAllowed) {
      if (status === "success") {
        ...
        // TODO: Tambahkan logika update saldo user berdasarkan ref
