...
app.post('/callback.php', (req, res) => {
    const jsonBody = req.body;
    const rawBody = JSON.stringify(jsonBody);

    const localSignature = crypto.createHmac('sha256', PRIVATE_KEY)
        .update(rawBody)
        .digest('hex');
    ...
    fs.appendFileSync(CALLBACK_LOG, JSON.stringify(logEntry) + '\n');

    if (!callbackSignature || localSignature !== callbackSignature) {
        return res.status(400).json({status: "error", message: "Invalid signature"});
    }

    if (jsonBody.status === "PAID") {
        const sqlite3 = require('sqlite3').verbose();
        const db = new sqlite3.Database('/root/BotVPN2/sellvpn.db');
        db.run(`UPDATE pending_deposits SET status = 'success' WHERE unique_code = ? AND status = 'pending'`,
            [jsonBody.reference], function(err){
                if(err) console.error(err.message);
                else console.log(`✅ Transaksi ${jsonBody.reference} berhasil diupdate`);
            });
        db.close();
    }
